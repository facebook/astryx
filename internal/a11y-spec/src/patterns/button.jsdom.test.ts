 ...
 *
 * SYNC: The Chromium half of this proof is ./button.chromium.spec.ts. Both read
...
const observableHere = BUTTON_PATTERN.expectations.filter(expectation =>
...
async function resultsFor(target: ButtonFixture) {
  ...
  const result = await runBinding({
    ...
    activations: async () => (window as ActivationWindow).__activations ?? 0,
  });
...
describe('button contract — completeness', () => {
  it('answers every completeness dimension', () => {
    expect(unansweredDimensions(BUTTON_PATTERN)).toEqual([]);
  });

  it('gives every expectation at least one deliberately violating fixture', () => {
    const missing = BUTTON_PATTERN.expectations
      .filter(
        expectation => (BUTTON_MUTATIONS[expectation.id] ?? []).length === 0,
      )
  ...
  it('has an expectation for every violating fixture it records', () => {
    const ids = new Set(
      BUTTON_PATTERN.expectations.map(expectation => expectation.id),
    );
    const orphans = Object.keys(BUTTON_MUTATIONS).filter(id => !ids.has(id));
    expect(orphans).toEqual([]);
  });

  it('names every fixture it records a mutation against', () => {
    for (const fixtures of Object.values(BUTTON_MUTATIONS)) {
...
describe('button contract — the jsdom harness reports what it cannot see', () => {
  it('reports an expectation above the DOM layer as unrun, never as a pass', async () => {
    const results = await resultsFor(fixture('conforming-native'));
...
describe.each(observableHere.map(expectation => [expectation.id] as const))(
  '%s',
  id => {
    const expectation = BUTTON_PATTERN.expectations.find(
    ...
    it.each((BUTTON_MUTATIONS[id] ?? []).map(name => [name] as const))(
    ...
    it('carries its id and its normative source into every failure it reports', async () => {
      const [violating] = BUTTON_MUTATIONS[id] ?? [];
