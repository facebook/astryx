...
| `hasCollapsedControls` | `boolean` | `true` |
| `hasCollapsedLabel` | `boolean` | `true` |

`activeStep` is the zero-based index of the current step. `onStepClick` makes
steps clickable for non-linear navigation.

`hasCollapsedControls` sets whether the collapsed stepper shows Previous/Next
controls beneath the track; they only ever appear when `onStepClick` is set.
`hasCollapsedLabel` sets whether the collapsed stepper names the current step
...
```tsx
<Stepper activeStep={step} onStepClick={setStep} hasCollapsedControls={false}>
  <Step step={0} label="Cart" />
  <Step step={1} label="Address" />
  <Step step={2} label="Payment" />
</Stepper>
...
Use `hasCollapsedControls` / `hasCollapsedLabel` instead.
