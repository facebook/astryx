...
`hasControls` sets whether the collapsed stepper shows Previous/Next
controls beneath the track; they only ever appear when `onStepClick` is set.
`hasLabel` sets whether the collapsed stepper names the current step
...
```tsx
<Stepper activeStep={step} onStepClick={setStep} hasControls={false}>
  <Step step={0} label="Cart" />
  <Step step={1} label="Address" />
  <Step step={2} label="Payment" />
</Stepper>
...
Use `hasControls` / `hasLabel` instead.
