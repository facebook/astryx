...
People who depend on assistive technology should get the focus and announcement outcome Astryx claims, not only plausible test-DOM markup. Astryx uses real AT/browser checks when that outcome depends on focus, announcement behavior, virtual-cursor entry, or a known AT/browser divergence.
...
| ID  | Rule                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| --- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| FR1 | The gate follows the claimed user outcome. Real AT/browser evidence is required when the intended user outcome depends on what focus or assistive technology does. A semantic code change alone is not the trigger; the trigger is a claim that cannot be proved below the real-AT layer in the evidence model.                                                                                                                                              |
| FR2 | Focus-destination changes trigger when focus is the outcome. Real AT/browser verification is required when a change changes or newly relies on the element receiving programmatic, initial, restored, composite, or after-dismissal focus, and the person must identify or continue the task from that destination. Focus-ring styling, a refactor preserving the same browser focus target, or a static `tabIndex` correction does not trigger by itself.   |
...
| ID   | Rule                                                                                                                                                                                                                                                                                                   |
| ---- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| FR12 | Desktop-neutral claims use two independent stacks. A focus or announcement outcome claimed across supported desktop browsers uses at least NVDA + Chrome on Windows and VoiceOver + Safari on macOS. A platform-specific claim uses its matching pairing only unless FR14 expands it.                  |
...
| ID   | Rule                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| ---- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| FR17 | Merge and stable release are separate gates. When intended user impact depends on focus, announcement or virtual-cursor behavior, or a known AT/browser divergence under FR2–FR6, the implementation may merge to `main` once its required automated and real-browser evidence passes, its real-AT matrix and scenario are named, and a durable release-block record exists. Preview and prerelease builds may carry the pending change. The first stable release containing it MUST NOT publish while the block is pending, failed, or expired. |
...
- Supported feature/engine floor: the matrix follows the browser and platform support of the affected Astryx component or package.
...
| Affected release | The first stable release containing that commit; the version is not yet known.                                                                                                                                                                         |
| Trigger          | FR2 and FR3. The intended result depends on the focused role and name being announced on entry.                                                                                                                                                        |
| Scenario         | Open the `Read-only dialog focus` story, confirm the dialog name and role are announced at the focused container, then confirm the first Tab enters the dialog's control cycle rather than moving to unrelated content.                                |

Its merge-time unit, DOM, axe, and browser evidence can support merging the semantic change. Because the intended outcome depends on what is announced at that focus destination, the first stable release waits for the receipts below or an explicit current exception. Existing axe and browser checks remain required where they prove assigned claims; neither clears an AT-triggered release block by itself.
...
Both rows block that stable release until they have passing durable receipts or are covered by a current owner-recorded exception.

This accepted spec creates no runtime or process implementation. A later atomic change may add contributor workflow or release mechanics that project this contract without changing it.
...
| --------- | ---------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------ |
| FR1–FR8   | Classification review against the change's claimed user outcome. | static semantic fix; container autofocus; focus return; live update; virtual-cursor entry; known divergence | AT-dependent outcome ships without a gate, or unrelated accessibility fix is forced through a universal AT matrix. |
...
**Decision:** FR1–FR8 require real AT/browser verification for focus, announcement behavior, virtual-cursor entry, or known divergence; other accessibility changes use the evidence layer that directly observes their claim.
...
**Decision:** Desktop-neutral focus and announcement claims use NVDA + Chrome on Windows and VoiceOver + Safari on macOS. Mobile, platform-specific, and known-divergent claims select or expand that matrix according to the affected outcome. Passing results make no universal compatibility claim.
...
**Decision:** Under FR2–FR6, implementation may merge after required automated and real-browser evidence plus a durable named follow-up. The first stable release remains blocked until the named real AT/browser matrix passes or a spec owner records a narrow, expiring exception.
