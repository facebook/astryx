...
| Markdown-derived Outline       | Existing calls omit options and retain released headings except corrected Unicode/collision ids.           | Pass the same `sourceIds`, `autolink`, `math`, `plugins`, and finality to Markdown and `parseOutlineFromMarkdown`/`useOutlineFromMarkdown`.                                            | The caller owns which document/parser configuration both surfaces represent; Core then owns shared text/slug identity.                           |
