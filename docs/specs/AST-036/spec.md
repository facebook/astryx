...
| Compatibility        | Omitted plugins and `plugins={[]}` are semantically the same empty pipeline. Core may skip empty preparation and allocation, but no separate behavior model exists. Existing `components`, `inlinePlugins`, math, citations, autolinks, and released parser signatures retain their meaning. Explicit Core footnotes preserve default runtime values while adding two cases to the public canonical AST unions. |
...
| `image`, `url`, `alt`, `title`              | Revalidated resource URL, required text alternative, and optional string title.                                                                                                                                      | Bypassing resource policy, removing required alternative text, or using a non-string title.                                                                                                                         |
...
| Astryx flow image                           | Preserves released standalone-image block behavior with `url`, `alt`, and optional `title`; phrasing images retain normal MDAST placement.                                                                           | Treating the flow extension as portable MDAST without adapter evidence.                                                                                                                                             |
...
| Core footnotes                              | Shared canonical unions contain MDAST-aligned `footnoteReference` and top-level `footnoteDefinition` nodes; enabled block parses may emit them and explicit released opt-ins use recursive footnote result families. | Plugin minting/duplication/retyping/relabeling, nested definitions, references inside links or definitions, swallowed unresolved source, changed heading IDs outside resolved-reference text, or Remark acceptance. |
| Other MDAST nodes                           | Unsupported unless a later current contract adds them.                                                                                                                                                               | `html`, ordinary definitions/references, frontmatter, directives, MDX, and other unknown nodes fail closed in the Remark adapter.                                                                                   |
...
| Footnotes                       | Move/remove parsed nodes; transform definition descendants under Core rules while definitions remain direct root children and references remain outside links and definitions. | Mint/duplicate/retype/edit IDs or labels; invalid nesting; renderer-owned numbering or navigation.             |
...
- **FR3 — Default behavior and released projections stay compatible.** Existing `components`, `inlinePlugins`, citations, opt-in GFM autolinking, math, sources, released parser result signatures and fields, DOM, accessibility, heading/Outline identity, streaming reuse, and renderer precedence retain their released meaning and behavior when new syntax is omitted. Contract-restoring Unicode normalization and duplicate-ID prevention follow FR16. An explicitly enabled Core syntax may add its canonical node kinds to the shared public canonical unions; exhaustive canonical-AST consumers then add those cases as a documented source migration even though default runtime AST values remain unchanged. Opting into plugins adds behavior only for that callsite.
...
- **FR9 — Canonical AST is MDAST-aligned, strictly typed, and reachable.** Parsing, transforms, rendering, and Outline projection use one stable Unist-shaped subset with complete available source positions. `parseMarkdownAst()` and `parseInlineAst()` return that canonical tree from the server-safe `@astryxdesign/core/Markdown/parser` subpath and the Markdown component barrel. Public node-kind traversal narrows callbacks by discriminant, including generic plugin extensions. Released parser functions retain their existing result types and values until a separately approved migration; adding a canonical built-in node kind explicitly widens the canonical unions and their exhaustive-switch surface.
...
- **FR16 — One heading projection.** Markdown and Markdown-derived Outline use the same parser options, syntax list, transform list, text projection, NFC-normalized Unicode slugger, document-global collision allocator, and released top-level heading traversal. `prepareMarkdownDocument()` may materialize that root and projection once for both surfaces. Renderer output cannot change identity. A heading nested in an extension container behaves like one nested in a blockquote: it does not create an Outline entry or extend heading-ID traversal unless a later owner contract permits it.
...
- **FR41 — Footnotes, definitions, and references stay Core scope.** They are not a plugin surface: their identity is document-level ID allocation, collision handling, and back-reference navigation that Core owns under FR15 and FR16, and a plugin owning them would fork the identity system Markdown and Outline share. The separately owned `module:Markdown/footnotes` contract may add explicit Core syntax and rendering while preserving the released default path. Ordinary definitions/references remain unsupported, and the Remark adapter continues to reject all footnote nodes rather than translating or approximating them.
...
Block, inline, incremental, canonical-AST, Markdown, and Outline entrypoints accept the same ordered plugin configuration and infer the same extension-node union while retaining their released signatures and projected result shapes. The canonical entrypoints return the canonical tree directly; released entrypoints keep their projection. Explicit Core footnotes are block-only: Markdown, `parseMarkdown`, incremental block parsing, prepared documents, and source-derived Outline accept the matching option, while inline entrypoints do not. The server-safe parser entry exposes the same behavior without a client boundary. The server-safe `Markdown/document` entry may prepare one block root, transform result, heading projection, and Core-footnote projection for reuse by Markdown and Outline; it is an opaque in-process value rather than an RSC serialization format. The Markdown component barrel re-exports both APIs for client code. Inline transforms must satisfy the one-paragraph phrasing constraint. Transform-only updates cannot increase syntax tokenizer calls, mutate prior snapshots, or remount unchanged settled output beyond current behavior.
...
| Markdown-derived Outline       | Existing calls omit options and retain released headings except corrected Unicode/collision ids.           | Prepare a finished block document through `Markdown/document`, pass it as `Markdown document`, and pass its readonly `outline` to `Outline`; use the source helper for streaming.      | The caller owns which document/parser configuration both surfaces represent; Core then owns shared text/slug identity and reuse.                 |
...
| FR40–FR41            | First-party records, Core-footnote ownership, and adapter rejection fixtures                                                                                  | Soft breaks, entity references, callouts, omitted/enabled footnotes, plain definitions, Remark nodes                             | First-party privilege, plugin ownership, default widening, or Remark acceptance fails.                                           |
...
These concepts require document-level ID allocation, collision handling, and back-reference navigation shared with Markdown identity. The separately owned `module:Markdown/footnotes` contract adds them only as explicit Core block syntax and rendering while preserving default parser values and DOM. The two node kinds join the shared public canonical unions so transforms and prepared documents keep one tree; this is a documented exhaustiveness migration for canonical-AST TypeScript consumers, while released parser result unions remain narrow unless explicitly enabled. Ordinary definition/reference syntax remains unsupported, and the Remark adapter continues to reject footnote nodes rather than approximating them.

Rejected: first- or third-party footnote plugins; adapter translation into ordinary links; implicit document-variant enablement; changing heading IDs outside the text removed by explicitly resolved references; leaving the limitation undocumented.
