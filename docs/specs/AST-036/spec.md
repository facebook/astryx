...
| Core footnotes                              | Explicit Core options add MDAST-aligned reference and top-level definition nodes with Core-owned identity.                                                                                       | Plugin ownership, nested declarations, swallowed unresolved source, changed heading IDs, or Remark acceptance.                     |
...
| Footnotes                       | Move/remove parsed nodes; transform definition descendants under Core rules.                                | Mint/duplicate/edit IDs or labels; renderer-owned numbering or navigation.                                     |
...
- **FR3 — Zero breaking changes.** Existing `components`, `inlinePlugins`, citations, opt-in GFM autolinking, math, sources, parser signatures, fields, optionality, mutability, no-plugin result types, DOM, accessibility, heading/Outline identity, streaming reuse, and renderer precedence retain their released meaning and behavior. Contract-restoring Unicode normalization and duplicate-ID prevention follow FR16. Opting into plugins adds behavior only for that callsite.
...
These concepts require document-level ID allocation, collision handling, and back-reference navigation shared with Markdown identity. The separately owned `module:Markdown/footnotes` contract may add them only as explicit Core syntax and rendering while preserving the default parser and DOM. Ordinary definition/reference syntax remains unsupported, and the Remark adapter continues to reject footnote nodes rather than approximating them.

Rejected: first- or third-party footnote plugins; adapter translation into ordinary links; implicit document-variant enablement; changing existing heading IDs; leaving the limitation undocumented.
