 ...
 * that Node cannot resolve. So each runtime ESM target is also imported. This runs
 * after the build, so a forward reference to something a later pipeline stage
...
/**
 * Runtime ESM targets worth actually importing. `.cjs` and `require`-condition
 * targets are skipped (loading CJS here would fire side effects under a
 * different module system), as are type declarations and assets, which Node
 * cannot import as modules anyway.
 */
const IMPORTABLE = /\.(js|mjs)$/;
const NON_RUNTIME_CONDITIONS = new Set(['types', 'require', 'default']);
...
function checkExportsValue(pkgDir, pkgName, exportKey, value, parentCondition) {
  ...
  if (typeof value === 'string') {
    ...
    if (!ok) {
      ...
      !NON_RUNTIME_CONDITIONS.has(parentCondition)
    ) {
      importTargets.push({
        pkgName,
...
if (importFailures === 0 && importTargets.length > 0) {
  console.log(`${importTargets.length} runtime export target(s) imported cleanly.`);
}
...
if (failures > 0) {
  ...
  console.log('All exports resolve to existing files and load cleanly.');
}
