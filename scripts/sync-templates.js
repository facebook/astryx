 ...
 * 3. A theme doc registry (apps/sandbox/src/generated/themeDocRegistry.ts)
 *    from packages/themes/{name}/theme.doc.mjs + package.json
 *
...
async function discoverThemes() {
  ...
  for (const dir of dirs) {
    const docPath = path.join(THEMES_DIR, dir.name, 'theme.doc.mjs');
    const pkgPath = path.join(THEMES_DIR, dir.name, 'package.json');

    if (!fs.existsSync(docPath)) continue;
    if (!fs.existsSync(pkgPath)) {
      console.warn(`  skip theme ${dir.name}/ (has doc but missing package.json)`);
      continue;
    }

    const pkg = JSON.parse(fs.readFileSync(pkgPath, 'utf-8'));
    if (pkg.private) {
      console.log(`  skip theme ${dir.name}/ (private)`);
      continue;
    }
...
function generateThemeRegistry(themes) {
  const entries = themes.map(t => `  {
    ...
    npmName: ${JSON.stringify(t.npmName)},
    version: ${JSON.stringify(t.version)},
    description: ${JSON.stringify(t.description)},
    characteristics: ${JSON.stringify(t.characteristics, null, 6).replace(/\n/g, '\n    ')},
  }`);
...
export interface ThemeCharacteristic {
  label: string;
  value: string;
  detail?: string;
}

export interface ThemeDocEntry {
  slug: string;
  npmName: string;
  version: string;
  description: string;
...
export const themeDocs: ThemeDocEntry[] = [
${entries.join(',\n')}
];
`;
}

function generateThemeImports(themes) {
  const imports = themes.map(
    t => `import {${t.slug}Theme} from '${t.npmName}';`
  ).join('\n');

  const entries = themes.map(
    t => `  'pkg-theme-${t.slug}': ${t.slug}Theme,`
  ).join('\n');

  return `/**
 * AUTO-GENERATED by scripts/sync-templates.js — do not edit.
 * Provides theme object imports for the docsite.
 */
...
async function main() {
  ...
  const [{pages, blocks, all}, themes] = await Promise.all([
    discoverTemplates(),
    discoverThemes(),
  ]);
  console.log(`  found ${pages.length} page(s), ${blocks.length} block(s), ${themes.length} theme(s)`);
  ...
  const themeRegistryPath = path.join(GENERATED_DIR, 'themeDocRegistry.ts');
  fs.writeFileSync(themeRegistryPath, generateThemeRegistry(themes));
  console.log(`  wrote ${path.relative(ROOT, themeRegistryPath)}`);

  const themeImportsPath = path.join(GENERATED_DIR, 'themeImports.ts');
  fs.writeFileSync(themeImportsPath, generateThemeImports(themes));
  console.log(`  wrote ${path.relative(ROOT, themeImportsPath)}`);
