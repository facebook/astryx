...
For component-specific documentation, see the `{Name}.doc.tsx` file in each component directory under `packages/core/src/` (e.g. `Button/Button.doc.tsx`). These are typed TypeScript files exporting a `ComponentDoc` object from `packages/core/src/docs-types.ts`.
