...
## StyleX CSS Capability Reference (v0.17.5)

> Auto-generated by `node internal/stylex-capabilities/scan.mjs`. Re-run when bumping StyleX version.

StyleX supports these CSS features in `stylex.create()`. Use CSS-native solutions — don't build JS workarounds for features that are already supported.

**At-Rules:** `@media`, `@supports`, `@container` (+ named), `@layer`, `@starting-style` ✅
...
**Compound Selectors:** `::backdrop` + condition, RTL via `:is([dir="rtl"] *)`, nested `@media` + pseudo ✅

**CSS Values:** `var()`, `calc()`, `clamp()`, `light-dark()`, `color-mix()`, `container-type`, `container-name` ✅

**Transitions & Animations:** `transition` (shorthand + individual), `transitionBehavior: allow-discrete`, `animation`, `stylex.keyframes` ✅

**Dynamic Styles:** Functions in `stylex.create` for runtime values ✅

**Variables:** `stylex.defineVars`, `stylex.createTheme` ✅ (require `.stylex.ts` files)

**Logical Properties:** `paddingInline/Block`, `insetInlineStart/End`, `borderInlineStart`, `marginInlineStart` ✅

**Layout:** `grid`, `flex` + `gap`, `aspect-ratio`, `overscrollBehavior` ✅

When in doubt, test: `node internal/stylex-capabilities/scan.mjs`
