...
| Behavior                | The nearest pointer x selects one datum; the card, enabled SVG indicator, and eligible dots update together. The card uses Layer fixed mode and browser top-layer promotion when supported.             |
| End-user impact         | In supported browsers, pointer users can read the same chart details when the chart is clipped or inside browser top-layer UI; the reduced fallback preserves ordinary visibility.                      |
...
| Governing rules         | `spec:AST-027/FR7–FR8`, `architecture:layer-runtime/INV1–INV3`, `spec:AST-013/FR7–FR10`, `architecture:component-test-sufficiency/INV1–INV11`, and `spec:AST-029/DEC-4`.                                |
...
| FR6 | A visible card MUST use `useLayer({mode: 'fixed'})`; supported browsers MUST receive native top-layer promotion, while browsers without the Popover API MUST use Layer's reduced visibility fallback. A body portal or numeric z-index is not a substitute.                                                    | `spec:AST-027/FR7–FR8`; `architecture:layer-runtime/INV1–INV3`; `spec:AST-013/FR7–FR8` | settled            |
...
- **ORD1 — Pointer selection.** Receive Chart pointer event → resolve nearest data index → derive datum and rows → resolve custom or default card content → show or hide Layer → measure the final card geometry and clamp its coordinates.
...
| ---------------- | ------------------------------------------------------------------------------------------------------ | --------------------------------------------------------------- | -------------- | ------------------ |
| Layer host       | Use native top-layer stacking when available and preserve ordinary visibility in the reduced fallback. | prescribed by `spec:AST-027/FR7–FR8` and `spec:AST-013/FR7–FR8` | overlay        | FR6                |
...
| FR6      | red/green Layer host and reduced-fallback tests; modal Storybook play assertion | closed, fallback open/closed, hovered inside native modal | Removing Layer loses native top-layer state, or the fallback flashes/stays visible.  | `audit:ChartTooltip/layering`      |
