...
import {XDSSchedule} from './XDSSchedule';
import {createXDSScheduleDayView} from './DayView';
import {createXDSScheduleListView} from './ListView';
import {createXDSScheduleMonthlyView} from './MonthlyView';
import {createXDSScheduleWeeklyView} from './WeeklyView';
import {sortEvents} from './dateMath';
import {useXDSScheduleViewSelectorPlugin} from './plugins/ViewSelectorPlugin';
import type {
  CalendarEvent,
  Instant,
  XDSScheduleCategory,
...
describe('XDSSchedule', () => {
  ...
  it('filters array events to the active view range', async () => {
    render(
      <XDSSchedule
        view={createXDSScheduleMonthlyView()}
        events={events}
  ...
  it('renders list view grouped by localized day', async () => {
    ...
    render(
      <XDSSchedule
        view={createXDSScheduleListView({days: 7})}
        events={listEvents}
        categories={categories}
        date={Date.UTC(2026, 4, 13)}
        timezoneID="UTC"
  ...
  it('renders weekly view with the same month title as monthly view', () => {
    render(
      <XDSSchedule
        view={createXDSScheduleWeeklyView()}
        events={events}
        categories={categories}
        date={Date.UTC(2026, 4, 13)}
        focusDate={Date.UTC(2026, 4, 13)}
  ...
  it('exposes time grid views as ARIA grids', () => {
    render(
      <XDSSchedule
        view={createXDSScheduleDayView({minHour: 8, maxHour: 10})}
        events={events}
        categories={categories}
        date={Date.UTC(2026, 4, 13)}
        focusDate={Date.UTC(2026, 4, 13)}
  ...
  it('exposes timed events in the accessible time grid cells', () => {
    render(
      <XDSSchedule
        view={createXDSScheduleDayView({minHour: 16, maxHour: 17})}
        events={events}
        categories={categories}
        date={Date.UTC(2026, 4, 13)}
        timezoneID="UTC"
  ...
  it('allows plugins to customize header slots', () => {
    const plugin: XDSSchedulePlugin = {
  ...
  it('renders a view selector plugin in the header end slot', () => {
    ...
    const dayView = createXDSScheduleDayView();
    const monthView = createXDSScheduleMonthlyView();
    const viewOptions = [
