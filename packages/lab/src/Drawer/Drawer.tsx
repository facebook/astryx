...
/** Which edge a drawer is anchored to. Mirrors the `side` prop's values. */
...
//
// `side` is carried because Escape order and RECEDE depth are two different
// questions of this one registry. Escape is chronological across every open
// drawer — the last one opened is the one Escape closes, wherever it sits.
// Receding is not: a drawer only recedes behind a drawer that actually covers
// it, and two drawers anchored to opposite edges never cover each other.
...
// Subscribers to stack membership. A buried drawer has to know how many
// drawers sit ON TOP of it, and that answer changes when a SIBLING opens or
// closes rather than when this drawer re-renders — so the registry publishes
// changes instead of each drawer polling. `useSyncExternalStore` then keeps
// depth in sync without a render-phase read of module state.
...
/**
 * How many drawers are stacked above `id` **on its own edge**. 0 for the top
 * of that edge's stack, and for a drawer that is not open.
 *
 * Scoped to one edge on purpose. A drawer recedes to say "you are on top of
 * me"; only a drawer that actually covers it can say that. A `side="start"`
 * drawer and a `side="end"` drawer sit side by side and never overlap, so
 * counting the whole registry would shrink an inspector because something
 * unrelated opened against the opposite edge.
 */
function countDrawersAbove(id: string): number {
  ...
  const {side} = openDrawerStack[index];
  let above = 0;
  for (let cursor = index + 1; cursor < openDrawerStack.length; cursor += 1) {
    if (openDrawerStack[cursor].side === side) {
      above += 1;
    }
  }
  return above;
...
function registerDrawer(
  ...
  openDrawerStack.push({id, close, side});
...
/**
 * Update an open drawer's edge in place.
 *
 * In place, rather than re-registering, because the registry position IS the
 * stacking order: re-adding would promote a buried drawer to the top of the
 * stack, and Escape would then close the panel the user cannot see instead of
 * the one in front.
 */
function setDrawerSide(id: string, side: DrawerSide): void {
  const entry = openDrawerStack.find(candidate => candidate.id === id);
...
const styles = stylex.create({
  dialog: {
    ...
    outline: 'none',
    // The public geometry of the stack, declared on the element that carries
    // the `drawer` theme target so a theme can retune the effect without
    // forking the component — the same door `--spinner-diameter` opens, e.g.
    // drawer: { base: { '--drawer-stack-peek': '24px' } }.
    //
    // The scale step is a unitless ratio per level rather than a px inset like
    // the EPS Sheet this is lifted from. An inset in px has to be divided by
    // the measured panel size to become a scale, which costs a ResizeObserver
    // and makes the recede shallower on tall screens than on short ones; a
    // ratio is resolution-independent and needs no measurement.
    '--drawer-stack-peek': '40px',
    '--drawer-stack-scale-step': '0.04',
    '--drawer-stack-min-scale': '0.8',
    '--_drawer-stack-radius': radiusVars['--radius-element'],
    ...
    display: 'none',
    // `border-radius` rides along because a drawer that recedes behind a child
    // rounds its corners on the way back, and snapping that corner would read
    // as a different panel rather than the same one moving.
...
const dynamicStyles = stylex.create({
  ...
  // The recede: a drawer with `depth` drawers stacked on top of it withdraws
  // toward its own edge and shrinks, so the stack reads as layered pages
  // rather than one panel replacing another.
  //
  ...
  // pinned to that same edge so the shrink reads as a withdrawal into the page
  ...
  stackRecede: (depth: number, isEnd: boolean) => ({
    ...
    borderRadius: 'var(--_drawer-stack-radius)',
  }),
});
...
export interface DrawerProps extends BaseProps<HTMLDialogElement> {
   ...
   * default a buried drawer withdraws toward its own edge and shrinks a little
   * per level, so the stack reads as layered pages with each one's leading
   * edge still visible — the user can see what they came from and how deep
   * they are, instead of one panel silently replacing another.
   *
   * Only drawers on the SAME `side` stack: a `side="start"` drawer and a
   * `side="end"` drawer sit beside each other and never bury one another.
   *
   * Set `false` to keep the panel at rest while others open over it. The
   * geometry is themeable rather than per-call: `--drawer-stack-peek`,
   * `--drawer-stack-scale-step` and `--drawer-stack-min-scale` on the `drawer`
   * theme target.
   * @default true
   */
...
export function Drawer({
  ...
  // `side` is read through the anchor ref and is deliberately NOT a
  // dependency: re-registering would move this drawer to the end of the
  // registry, which is the stacking order, so a buried drawer that changed
  // edge would become the one Escape closes. The effect below updates the edge
  // in place instead.
  useEffect(() => {
    if (!isOpen) {
      return;
    }
    const z = registerDrawer(
      drawerId,
  ...
  useEffect(() => {
    ...
    setDrawerSide(drawerId, side);
  }, [isOpen, drawerId, side]);

  // How many drawers sit on top of this one, on this drawer's own edge. Read
  // from the same registry that already orders the stack, rather than from
  // React nesting: the documented contract is that sibling drawers stack
  // (never nest them), so a context walking the tree would find nothing to
  // walk. The server snapshot is 0 — nothing is open during SSR, so the first
  // client paint matches the markup.
