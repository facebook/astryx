...
export const docsDense = {
  description: 'combined date + time picker; calendar popover + time input in single flow',
  usage: {
    bestPractices: [
      { guidance: true, description: 'Clear labels + descriptions so users understand expected datetime' },
      { guidance: true, description: 'Use min + max to restrict selectable datetimes to valid ranges' },
      { guidance: true, description: 'Use hasClear when datetime optional so user can easily reset' },
      { guidance: true, description: 'Choose hourFormat (12h/24h) matching audience locale' },
      { guidance: false, description: 'Use DateTimeInput when only date needed — use DateInput instead' },
      { guidance: false, description: 'Use DateTimeInput when only time needed — use TimeInput instead' },
      { guidance: false, description: 'Hide label w/o surrounding context making field purpose obvious' },
    ],
  },
  propDescriptions: {
    ...
    isOptional: 'show "(optional)" indicator next to label',
    isRequired: 'mark field required',
    isDisabled: 'disable input + picker',
    value: 'selected datetime; ISO 8601 (YYYY-MM-DDTHH:MM or YYYY-MM-DDTHH:MM:SS)',
    onChange: 'callback on datetime change **(required)**',
    ...
    min: 'min selectable datetime; constrains both date + time',
    max: 'max selectable datetime; constrains both date + time',
    dateConstraints: 'custom constraint fns to disable specific dates',
    hasSeconds: 'include seconds in time portion',
    hourFormat: "'12h' shows AM/PM; '24h' uses 24-hour notation",
    timeIncrement: 'minutes to add/subtract w/ arrow keys in time input',
    hasClear: 'show clear button when datetime value set',
    placeholder: 'placeholder text when no datetime selected',
    size: 'input control size',
    status: 'error/warning/success status indicator w/ message',
