...
describe('SideNav resizable persistence edge cases (#4790)', () => {
  ...
  it('keeps expand clicks meaningful after a controlled parent refuses an expand', async () => {
    const user = userEvent.setup();
    const onCollapsedChange = vi.fn();
    render(
      <SideNav
    ...
    // The parent never accepts the change, so after the first click the
    // hook is expanded internally while SideNav still renders collapsed.
    await user.click(screen.getByRole('button', {name: 'Expand sidebar'}));
    expect(onCollapsedChange).toHaveBeenCalledTimes(1);
    expect(onCollapsedChange).toHaveBeenLastCalledWith(false);

    // The second click must still report an expand request.
    await user.click(screen.getByRole('button', {name: 'Expand sidebar'}));
    expect(onCollapsedChange).toHaveBeenCalledTimes(2);
    expect(onCollapsedChange).toHaveBeenLastCalledWith(false);
  });
