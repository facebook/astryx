...
export function Calendar({ref, ...props}: CalendarProps) {
  ...
  // the same provider-locale Intl formatting as the static caption so the
  ...
  const monthPickerOptions = useMemo(() => {
    ...
    return Array.from({length: 12}, (_, i) => {
      ...
      return {
        value: String(month),
        label: plainDateFormat(
          plainDateCreate(baseMonth.year, month, 1),
          {month: 'long'},
          locale,
        ),
  ...
  }, [showsMonthYearPickers, baseMonth.year, minPd, maxPd, locale]);
