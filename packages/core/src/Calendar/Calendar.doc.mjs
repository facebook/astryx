...
export const docs = {
  ...
  props: [
    ...
    {
      name: 'onChange',
      type: "((value: ISODateString, valueAsDate: Date) => void) | ((value: DateRange) => void)",
      description: 'Called when the user selects a date. In single mode receives the ISO string and a Date object; in range mode receives a DateRange ({ start, end }).',
    },
    {
      name: 'numberOfMonths',
      type: '1 | 2',
    ...
    },
    {
      ...
      description: 'Custom constraint functions. A date is disabled if ANY function returns false.',
    },
    {
      name: 'focusDate',
      type: 'ISODateString',
...
export const docsZh = {
  ...
  props: [
    ...
    {name: 'onChange', type: "((value: ISODateString, valueAsDate: Date) => void) | ((value: DateRange) => void)", description: '用户选择日期时调用。单选模式返回 ISO 字符串和 Date 对象；范围模式返回 DateRange ({ start, end })。'},
    ...
    {name: 'dateConstraints', type: 'ReadonlyArray<(date: Date) => boolean>', description: '自定义约束函数。任一函数返回 false 则该日期被禁用。'},
...
export const docsDense = {
  ...
  propDescriptions: {
    ...
    onChange: 'single mode: (value, valueAsDate) => void; range mode: (value: DateRange) => void',
    numberOfMonths: 'months to display',
    min: 'minimum selectable date',
    max: 'maximum selectable date',
    dateConstraints: 'constraint fns — date disabled if any returns false',
