...
describe('Calendar', () => {
  ...
  describe('month/year pickers', () => {
    ...
    it('labels the month picker in the provider locale, like the static caption', () => {
      render(
        <InternationalizationProvider locale="es-ES">
          <Calendar hasMonthYearPickers focusDate="2026-03-15" />
        </InternationalizationProvider>,
      );

      // Queried through the theme target rather than the "Month" name, which
      // is itself a translatable string.
      const month = document.querySelector(
        '.astryx-calendar-picker[data-picker="month"] [role="combobox"]',
      );
      expect(month).toHaveTextContent('marzo');
    });
    ...
    it('exposes calendar-picker as a themeable defineTheme target', () => {
      ...
      expect(css).toContain('color: var(--color-accent)');
      expect(css).toContain('.astryx-calendar-picker[data-picker="year"]');
    });
  });
});
