...
export const docsDense = {
  description: 'Hide/reveal content behind trigger button. Use in settings, FAQs, detail views. Wrap in XDSCollapsibleGroup for accordion. For custom collapsibles, use `useXDSCollapsible` hook (`xds hook useXDSCollapsible`).',
  usage: {
    bestPractices: [
      { guidance: true, description: 'Wrap each XDSCollapsible in XDSCard for visual separation in accordion layouts.' },
      { guidance: true, description: 'Use XDSCollapsibleGroup w/ type="single" for settings/FAQ pages; only one section open at a time.' },
      { guidance: true, description: 'Use type="multiple" when users need to compare content across sections.' },
      { guidance: true, description: 'Start sections open (defaultIsOpen) when content likely needed on first view; don\'t make users click for essential info.' },
      { guidance: false, description: 'Hide critical/required content behind collapsible; users may not discover it.' },
      { guidance: false, description: 'Nest collapsibles more than two levels deep; hard to find + navigate.' },
      { guidance: false, description: 'Use collapsible for single short paragraph; show text directly instead.' },
    ],
  },
  components: [
    {
      name: 'XDSCollapsible',
      displayName: 'Collapsible',
      description: 'Primitive making content collapsible; trigger toggles content visibility, manages own state or defers to parent XDSCollapsibleGroup.',
      propDescriptions: {
        trigger: 'Content in trigger area (always visible) **(required)**',
        defaultIsOpen: 'Default open state (uncontrolled)',
        isOpen: 'Controlled open state',
        onOpenChange: 'Callback when open state changes',
        value: 'ID for group coordination; required when inside XDSCollapsibleGroup',
      },
    },
    {
      name: 'XDSCollapsibleGroup',
      isHiddenFromOverview: true,
      displayName: 'Collapsible Group',
      description: 'Coordinates multiple XDSCollapsible instances; single (one open) or multiple (any open). No wrapper DOM.',
      propDescriptions: {
        ...
        onChange: 'Callback when open items set changes',
      },
    },
  ],
};
