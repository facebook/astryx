...
export const docs = {
  ...
  params: [
    ...
    {name: 'touchTrigger', type: "'auto' | 'always' | 'never'", description: 'How a tap behaves on touch, where there is no hover. auto opens the card only when the tap is not activating something (a link, a button); always is the two-tap contract on any trigger — first tap previews, second tap activates; never leaves taps alone.', default: "'auto'"},
