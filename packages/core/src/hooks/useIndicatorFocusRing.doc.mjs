...
export const docs = {
  ...
  params: [
    {
      ...
      description: 'Ref to an element wrapping only the indicator, so its single element child is unambiguously the thing to ring.',
      required: true,
    },
    {
      name: 'isDisabled',
  ...
  usage: {
    ...
    bestPractices: [
      { guidance: true, description: 'Wrap only the indicator in the ref\'d element; a wrapper holding label text would ring the whole row.' },
      { guidance: true, description: 'Spread focusProps on the element that contains the hidden input, not on the input itself.' },
      { guidance: false, description: 'Ask a themeable indicator to draw its own focus ring; a replacement that ignores the prop leaves the control with no visible focus.' },
    ],
  },
...
export const docsDense = {
  ...
  paramDescriptions: {
    containerRef: 'ref to element wrapping ONLY the indicator; its single element child gets the ring.',
    isDisabled: 'skip the ring; disabled control is not focusable.',
  },
  returnDescriptions: {
    focusProps: 'onFocus / onBlur to spread onto the element owning the focusable input.',
  },
  usage: {
    description:
      'Owner draws the ring so a theme-supplied indicator cannot ship a control w/ no visible focus (WCAG 2.4.7).',
    bestPractices: [
      { guidance: true, description: 'Wrap only the indicator in the ref\'d element; a wrapper w/ label text rings the whole row.' },
      { guidance: true, description: 'Spread focusProps on the element containing the hidden input, not the input.' },
      { guidance: false, description: 'Delegate the ring to a themeable indicator; a replacement that ignores it loses visible focus.' },
    ],
  },
};
