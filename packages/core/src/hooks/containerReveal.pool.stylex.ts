 ...
 *
 * WHY A POOL OF LITERAL BLOCKS: `stylex.when.ancestor(':hover', marker)` is
 * resolved by the StyleX compiler, which welds the marker's class into the
 * emitted selector (`.marker:hover *`). A marker and the styles referencing it
 * are therefore inseparable at build time — the marker cannot be passed as a
 * runtime argument, so the blocks cannot be produced by a loop or a helper and
 * must be written out literally per marker. A single shared marker would leak
 * hover/focus across NESTED reveal containers (an ancestor container's :hover
 * matches a descendant container's revealed content), so useContainerReveal
 * hands each concurrently mounted container a distinct marker from this pool
 * (see the free-list there).
 *
 * The reveal recipe hides content with `position: absolute` (removed from
 * layout flow, so it reserves no space) plus `opacity: 0`. Both keep the
 * content in the accessibility tree and tab order — unlike display/visibility
 * — so it stays reachable by keyboard and assistive tech while hidden.
 */
...
const s0 = stylex.create({
  ...
  // flow and fades in. Always shown on touch.
  reveal: {
...
// The compiled StyleX objects carry `when.ancestor` conditionals the public
// `StyleXStyles` type does not model, so the slot shape is inferred.
export const POOL = [
