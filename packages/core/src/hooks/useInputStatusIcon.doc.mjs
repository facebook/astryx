...
export const docs = {
  ...
  params: [
    ...
    },
    {
      ...
      type: 'boolean',
      description: 'Whether the input sits inside an InputGroup, which owns status rendering itself.',
      default: 'false',
    },
    {
      name: 'options.size',
  ...
  usage: {
    ...
    bestPractices: [
      { guidance: true, description: 'Render statusIcon inside the input container and merge describedBy into the control\'s aria-describedby list.' },
      { guidance: true, description: 'Let it decide when nothing should render — pass isInGroup and the variant through rather than branching at the call site.' },
      { guidance: false, description: 'Use it to convey the status by icon alone; the tooltip variant is the only one that carries the message, so the others still need a message box.' },
    ],
  },
  relatedComponents: ['TextInput', 'TextArea', 'NumberInput', 'DateInput', 'FileInput', 'FieldStatus', 'InputGroup'],
...
export const docsDense = {
  ...
  returnDescriptions: {
    statusIcon: 'node to render inside input container; null when nothing should render.',
    describedBy: 'id for aria-describedby, present exactly when the tooltip element is in the DOM.',
  },
  usage: {
    description:
      'Use when building a bordered input so all inputs behave the same per status; not for field-level messaging.',
    bestPractices: [
      { guidance: true, description: 'Render statusIcon in the input container; merge describedBy into aria-describedby.' },
      { guidance: true, description: 'Pass isInGroup + variant through; let the hook decide when nothing renders.' },
      { guidance: false, description: 'Rely on the icon alone for meaning; only the tooltip variant carries the message.' },
    ],
  },
};
