 ...
 *   11 private vars across 13 (component, var) sites were declared in source
...
const VARS_WITHOUT_DERIVED_MAPPING = new Set([
  ...
  // Indentation metrics: --tree-list-indent is the authorable step,
  // --_tree-indent the per-row distance TreeListItem computes from it.
  '--tree-list-indent',
  '--_tree-indent',
  ...
  '--_card-elevation',
  '--_card-ring',
  // Set per row on :focus-visible, not a resting value a theme writes.
  '--_tree-focus-outline',
  '--_tree-focus-outline-offset',
  // Consumed inside calc() alongside icon offsets rather than emitted as the
  // element's padding, so `padding` on the text-area target cannot drive it.
  // Making TextArea's inline padding themeable through the standard property
  // is a real change (derived[] entry + registry mapping), not a guard fix.
  '--_textarea-inline-padding',
]);
