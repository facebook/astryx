...
export const docsDense = {
  description:
    'Applies syntax highlighting colors to XDSCodeBlock + code components in subtree. Default: theme-level syntax colors via defineTheme({ syntax: ... }), derived from palette (--color-text-accent for keywords, --color-text-green for strings, etc.). XDSSyntaxTheme overrides per-region. 14 semantic tokens (keyword, string, comment, number, function, type, variable, operator, constant, tag, attribute, property, punctuation, background) validated against 11 community themes. Custom themes via defineSyntaxTheme(); support [light, dark] tuples for automatic color-scheme adaptation. Built-in presets: oneDarkPro, dracula, monokai, nord, tokyoNight, catppuccinMocha, githubLight, githubDark, solarizedLight, oneLight (import from @xds/core/theme/syntax).',
  usage: {
    bestPractices: [
      {guidance: true, description: 'Use syntax field in defineTheme() for app-wide code styling; use XDSSyntaxTheme only when specific section needs different look.'},
      {guidance: true, description: 'Pick built-in presets or create custom theme w/ defineSyntaxTheme() for brand-specific colors.'},
      ...
      {guidance: false, description: 'Wrap individual XDSCodeBlock instances w/ XDSSyntaxTheme; instead use syntaxTheme prop on XDSCodeBlock directly for per-instance overrides.'},
    ],
  },
  propDescriptions: {
    theme: 'Syntax highlighting theme; preset from @xds/core/theme/syntax or custom theme via defineSyntaxTheme() **(required)**',
  },
};
