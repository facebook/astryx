...
describe('Tokenizer', () => {
  ...
  describe('delimiters', () => {
    ...
    it('a delimiter commit clears the busy indicator of an in-flight search', async () => {
      // The commit empties the query and abandons the search, whose own
      // `finally` then declines to clear the busy state. The clear has to
      // reach the end lane too, or its spinner outlives the search.
      let resolveSearch: ((r: SearchableItem[]) => void) | undefined;
      const source: SearchSource = {
        search: async () =>
          new Promise<SearchableItem[]>(resolve => {
