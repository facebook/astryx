...
verified_by:
  [
    ...
    packages/core/src/Tokenizer/__tests__/TokenizerSelection.a11y.chromium.spec.ts,
...
| Compatibility/readiness | The default `hasEntriesOnFocus={false}` path and direct single-select BaseTypeahead remain unchanged. This decision is current and owner-approved; #6360 supplies the implementation, with exact-head interaction evidence required before merge. |
