...
export const docs = {
  ...
  usage: {
    description:
      'Toast shows a brief, non-blocking notification to confirm an action or surface transient information. Use it after saves, deletes, or status changes where the user needs feedback but not a decision.',
    bestPractices: [
      {guidance: true, description: 'Keep messages short — one sentence that tells the user what happened, like "Changes saved" or "Message sent".'},
      ...
      {guidance: false, description: 'Use a toast for critical errors that block the user — use Banner for persistent, in-context messaging that requires acknowledgment.'},
      {guidance: false, description: 'Put long or multi-line content in a toast — it disappears after 5 seconds and the user may not finish reading.'},
      {guidance: false, description: 'Show form validation errors as toasts — use inline field validation so the user can see exactly which field needs fixing.'},
    ],
    anatomy: [
...
export const docsZh = {
  ...
  usage: {
    description:
      'Toast 显示简短的非阻塞通知，用于确认操作或显示瞬态信息。用于保存、删除或状态变更后，用户需要反馈但不需要做决定的场景。',
    bestPractices: [
      {guidance: true, description: '保持消息简短——一句话告诉用户发生了什么，如"更改已保存"或"消息已发送"。'},
      ...
      {guidance: false, description: '对阻塞用户的严重错误使用 toast——使用 Banner 进行持久的上下文消息传递。'},
      {guidance: false, description: '在 toast 中放置长内容或多行内容——它会在5秒后消失，用户可能来不及阅读。'},
      {guidance: false, description: '将表单验证错误显示为 toast——使用内联字段验证让用户看到具体哪个字段需要修复。'},
    ],
    anatomy: [
...
export const docsDense = {
  ...
  usage: {
    description:
      'Brief non-blocking notification for action confirmations and transient info. Use after saves, deletes, or status changes needing feedback not decisions.',
    bestPractices: [
      {guidance: true, description: 'Short messages — one sentence: "Changes saved", "Message sent".'},
      ...
      {guidance: false, description: 'Critical blocking errors — use Banner for persistent in-context messaging.'},
      {guidance: false, description: 'Long/multi-line content — disappears in 5s, user may not finish reading.'},
      {guidance: false, description: 'Form validation errors — use inline field validation instead.'},
    ],
  },
  propDescriptions: {
