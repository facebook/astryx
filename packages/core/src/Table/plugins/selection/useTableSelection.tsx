...
export interface UseTableSelectionConfig<T extends Record<string, unknown>> {
  ...
  /**
   * Optional bulk-selection toolbar. The default fixed layout renders in flow as
   * a borderless full-bleed band. `layout: 'floating'` renders a rounded,
   * elevated bar 16px above the table without shifting it. Both layouts place
   * actions first and selection status plus clear-selection controls last. Omit
   * for the default selection behaviour with no toolbar.
   */
  ...
  return useMemo(
    (): TablePlugin<T> => ({
      ...
      transformScrollWrapper(props: ScrollWrapperRenderProps) {
        ...
        const toolbar = <TableBulkActionsToolbar config={bulkActions} />;
        if (bulkActions.layout === 'floating') {
          return {
            ...props,
            beforeScrollArea: (
              <>
                {props.beforeScrollArea}
                {toolbar}
              </>
            ),
          };
