...
| `XDSTableRow.tsx`        | `XDSTableRow`        | `<tr>` wrapper with context-aware styling + `extraStyles`        |
| `XDSTableCell.tsx`       | `XDSTableCell`       | `<td>` wrapper with context-aware styling + `extraStyles`        |
| `XDSTableHeaderCell.tsx` | `XDSTableHeaderCell` | `<th>` wrapper with context-aware header styling + `extraStyles` |
...
Each component accepts `extraStyles` for additional styles from plugin transforms (e.g., selected row background from the selection plugin).
