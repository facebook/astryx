...
labels and IDs. For a finished block document, `prepareMarkdownDocument()` exposes
the same projection as readonly `document.outline`; passing the document to
Markdown and its outline to Outline performs no second parse or transform pass.
...
| FR10 | A finished prepared document exposes its projected headings as readonly Outline items, and Markdown reuses that same canonical root and node-to-id map without parsing, transforming, or projecting again. Source-based utilities remain available for streaming and independent callers.                                                                                             | `component:Markdown` FR26       | implemented                         |
...
- Prepared finished documents materialize one canonical root and heading projection
  for both Markdown and Outline; consuming either performs no parser work.
...
| FR7–FR10 | Heading insert/remove parity, failure parity, streaming derivation, and prepared-document reuse | headings removed with/without sections; inserted synthetic headings; failing transform; growing source prefix; server derivation; one-parse prepared composition            | Outline/document disagreement, divergent fallback, a throw, stale IDs, streamed oscillation, or repeated prepared work.                 |
