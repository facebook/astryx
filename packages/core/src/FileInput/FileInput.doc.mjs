...
export const docsDense = {
  description: 'File upload w/ input/dropzone modes; single/multi selection + built-in type, size, count validation.',
  usage: {
    bestPractices: [
      {guidance: true, description: 'Always specify accept prop to guide users toward valid file types.'},
      {guidance: true, description: 'Use maxSize + maxFiles to prevent oversized uploads; component handles validation + error display automatically.'},
      {guidance: true, description: 'Add description to communicate constraints like file size limits/accepted formats.'},
      {guidance: true, description: 'Use changeAction for immediate upload workflows w/ optimistic UI.'},
      {guidance: false, description: "Don't use FileInput for directory/folder uploads — not supported in v1."},
      {guidance: false, description: "Don't avoid dropzone mode unless space is very constrained — drag-and-drop is expected interaction for file uploads."},
    ],
  },
  propDescriptions: {
    label: 'Accessible label **(required)**.',
    value: 'Currently selected file(s); controlled **(required)**.',
    onChange: 'Fired when files selected/removed **(required)**.',
    changeAction: 'Async change action (React 19 transitions); use for immediate upload on selection.',
    accept: 'Accepted file types; HTML accept format (e.g. "image/*", ".pdf,.doc").',
    isMultiple: 'Allow multi-file selection; when true, value + onChange use File[] instead of File.',
    maxSize: 'Max file size in bytes; exceeding files rejected w/ error status.',
    maxFiles: 'Max file count (only when isMultiple is true).',
    isLabelHidden: 'Visually hides label; keeps screen reader access.',
    description: 'Text between label + input.',
    isOptional: '"Optional" indicator next to label; mutually exclusive w/ isRequired.',
    isRequired: '"Required" indicator next to label + sets aria-required; mutually exclusive w/ isOptional.',
    isDisabled: 'Disables input; prevents interaction + dims element.',
    isLoading: 'Loading state w/ spinner + aria-busy.',
    placeholder: 'Text when no file selected; defaults to "Choose file"/"Choose files".',
    mode: "'input' (compact inline) / 'dropzone' (larger area w/ drag-and-drop).",
    status: 'Validation status; colored border + optional floating message. Error sets aria-invalid.',
