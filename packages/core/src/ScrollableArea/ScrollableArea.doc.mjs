...
export const docs = {
  ...
  props: [
    {
      ...
      description: 'Logical axis or axes where native scrolling is allowed.',
      default: "'block'",
    },
    {
      name: 'label',
    ...
    },
    {
      name: 'role',
      type: "'group' | 'region'",
      description: 'Semantics for the named viewport.',
      default: "'group'",
    },
    {
      name: 'overscroll',
    ...
    },
    {
      name: 'children',
      type: 'ReactNode',
      description: 'Content rendered inside the observed content box.',
    },
    {
      name: 'xstyle',
      type: 'StyleXStyles',
  ...
  theming: {
    targets: [{className: 'astryx-scrollable-area', visualProps: ['axis']}],
  },
};
...
export const docsDense = {
  description:
    'native logical-axis scroll viewport with conditional keyboard access, live edge state, and observed content geometry',
  usage: {
    description:
      'Use for a complete viewport/content composition; use useScrollableArea for existing structure.',
    bestPractices: [
      {guidance: true, description: 'Always provide a concise label.'},
      {
        guidance: true,
        description:
          'Prefer overscroll allow; contain only deliberate nested interactions.',
      },
      {
        guidance: true,
        description:
          'Use content padding props to publish inset; use isFullBleed only for viewport escape.',
      },
      {
        guidance: false,
        description: 'Nest another overflow wrapper around it.',
      },
    ],
    anatomy: [
      {
        name: 'Viewport',
        required: true,
        description: 'native scrolling, conditional tab stop, and theme target',
      },
      {
        name: 'Content box',
        required: true,
        description: 'real observed layout box',
      },
    ],
  },
  propDescriptions: {
    ...
    overscroll:
      "edge behavior: 'allow' (default) | 'contain' on effective axes only",
    padding:
      'content inset on every edge; defaults to 0 and publishes geometry',
