...
// ---------------------------------------------------------------------------
// Legacy flat-token API (absolute offsets)
// ---------------------------------------------------------------------------

/**
 * @deprecated Use `tokenize()` which returns per-line tokens.
 * This function returns tokens with absolute character offsets
 * for backwards compatibility with CodeEditor.
 */
export function tokenizeFlat(code: string, language: string): Array<{type: string; start: number; end: number}> {
  const langDef = buildLanguage(language);
  if (!langDef) return [];
...
/**
 * @deprecated Use `tokenizeAsync()` which returns per-line tokens.
 */
export async function tokenizeAsyncFlat(
  code: string,
  language: string,
  signal?: AbortSignal,
): Promise<Array<{type: string; start: number; end: number}>> {
  const langDef = buildLanguage(language);
  if (!langDef) return [];
  ...
  while (pos < code.length) {
    ...
    if (charsInChunk >= ASYNC_CHUNK_SIZE) {
      charsInChunk = 0;
      await yieldToMain();
    }
  }
