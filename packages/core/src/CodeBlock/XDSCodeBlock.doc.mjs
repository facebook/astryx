  ...
  components: [
    {
      ...
      props: [
        ...
        {name: 'hasHeader', type: 'boolean', description: 'Show the header bar (title, language label, copy button row). Set to false for a compact display with reduced padding — useful when embedding inside chat tool-call results or tight layouts. When false, the copy button renders as an absolute-positioned overlay if hasCopyButton is true.', default: 'true'},
  ...
  components: [
    {
      ...
      propDescriptions: {
        ...
        hasHeader: '显示标题栏（标题、语言标签、复制按钮行）。设为 false 可获得紧凑显示和减少的内边距——适用于嵌入聊天工具调用结果或紧凑布局。',
        onCopy: '代码复制后的回调函数。',
  ...
  components: [
    {
      ...
      propDescriptions: {
        ...
        hasHeader: 'show header bar (title + language + copy button row); false = compact mode with reduced padding',
