...
export const docsZh = {
  ...
  props: [
    ...
    },
    {
      ...
      type: '(nextQuery: string) => string',
      description:
        '在编辑文本成为查询之前重写它。以输入框即将取得的值调用，返回值将被采用。在查询存储、onChangeQuery 触发和搜索调度之前运行。粘贴时以粘贴后的完整文本（剪贴板内容替换选区）调用，先于单行输入框剥离换行符；返回不同文本会取代原生插入。输入法组合结束后对最终文本再运行一次。组件自身引起的查询变化（如选择后的清空）和输入法组合过程中不会调用。Tokenizer 用它把分隔的值从输入框提取为标记。',
    },
    {
      name: 'onChangeQuery',
      type: '(query: string) => void',
...
export const docsDense = {
  ...
  propDescriptions: {
    ...
    transformQuery:
      'Rewrites edited input text before it becomes the query (runs before store/onChangeQuery/search; on paste gets the composed post-paste value pre-newline-strip; runs on IME text at composition end; not for self-made query changes or mid-IME). Tokenizer uses it to split delimited values into tokens.',
