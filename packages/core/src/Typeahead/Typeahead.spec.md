...
| Input busy          | `Typeahead.test.tsx` `input busy: isLoading and changeAction` suite                   | `isLoading`; pending or clear Action; settled; replaced mid-Action; source busy and input busy; disabled | A second Spinner appears, `aria-busy` drops while either meaning is busy, selection or clear bypasses `family:input-fields` FR6, or a focusable-disabled field selects from the keyboard (`family:input-fields` FR4). | `audit:Typeahead/accessibility` |
