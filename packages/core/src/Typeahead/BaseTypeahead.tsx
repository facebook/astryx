...
const BaseTypeaheadImpl = function BaseTypeahead<T extends SearchableItem>({
  ...
  const menuItems = useMemo(() => {
    ...
    return isTokenizer
      ? groupItems(projected, {ungroupedFirst: true}).flatMap(
          group => group.items,
        )
      : projected;
  }, [isTokenizer, projectResults, results]);
  ...
  /* eslint-disable @eslint-react/set-state-in-effect -- authoritative committed Tokenizer state must synchronously invalidate Base-owned pending work and popup state before paint */
  const invalidatePending = useCallback(
    ...
    [popover, searchSource, setLoading],
  );
  ...
  useIsomorphicLayoutEffect(() => {
    if (
      ...
      (isDisabled ||
        ...
        (results.length > 0 && menuItems.length === 0))
    ) {
      invalidatePending(true);
    }
  }, [isDisabled, projectResults, tokenizerMaxEntries, tokenizerSelectedCount]);
  ...
  const handleSelect = useCallback(
    (item: T) => {
      ...
      invalidatePending(!keepsTokenizerMenuOpen);
      onChange(item);
      setQuery('');
