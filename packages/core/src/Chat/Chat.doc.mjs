...
export const docs = {
  ...
  usage: {
    ...
    bestPractices: [
      ...
      { guidance: true, description: 'Wrap custom in-message content (cards, attachments, citations) in ChatMessageContent so it aligns with the bubble text column and can use the full message width.' },
    ...
    anatomy: [
      ...
      { name: 'Content', required: true, description: 'The message body: one or more ChatMessageBubble elements, or any free-form ReactNode like images or tool calls. Wrap non-bubble content in ChatMessageContent to align it with the bubble text column.' },
...
export const docsZh = {
  usage: {
    ...
    bestPractices: [
      ...
      { guidance: true, description: '将自定义消息内容（卡片、附件、引用）包裹在 ChatMessageContent 中，使其与气泡文本列对齐，并可使用消息列的全部宽度。' },
...
export const docsDense = {
  ...
  usage: {
    ...
    bestPractices: [
      ...
      { guidance: true, description: 'ChatMessageContent wraps custom content (cards/attachments); insets to bubble text column + full column width.' },
