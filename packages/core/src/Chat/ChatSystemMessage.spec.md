...
| Area                    | Contract                                                                                                                                                                                                           |
| ----------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Public contract         | The exported noninteractive chat row accepts required content, `default` or `divider` presentation, optional leading content, and supported root DOM, styling, and ref inputs.                                     |
| Behavior                | The default branch centers content and optional leading content in a status region. The divider branch delegates its labelled separator to Divider and currently does not render the optional `icon` input.        |
...
| Builder impact          | Builders choose between a plain status row and a labelled divider and provide concise, self-contained content. No new caller choice is introduced by this observational record.                                    |
| Compatibility/readiness | This draft records released observable behavior and adds documentation and evidence only. The unresolved divider/icon combination remains unchanged and requires an owner decision before either behavior changes. |
...
| Governing rules         | `architecture:public-component-api/INV1-INV9`; `architecture:component-theming-surface/INV2-INV6`; `architecture:component-test-sufficiency`; `spec:AST-009`; `spec:AST-020`; `spec:AST-029`.                      |
...
- Released default preserved: yes; this audit changes no runtime behavior or default.
- Compatibility class: observational documentation and evidence only.
...
- Placement of optional leading content in the default branch.
...
| Leading content  | omitted; caller-provided `ReactNode`                     | Renders before content in the default branch.                               | Default only in current implementation.   | Omitted   | caller and component                | Unsettled released surface | Divider currently ignores the input; see OQ1.       |
...
| --- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------- | ------------------------------------------------------- |
| FR1 | The default branch MUST render the required content in a centered, noninteractive status row and render optional leading content before it.                                             | Current source, released docs, tests, and examples                         | Verified current behavior                               |
| FR2 | The divider branch MUST render the required content through Divider as a named horizontal separator inside the component's status root.                                                 | Current source, released docs, Divider implementation, tests, and examples | Verified current behavior                               |
| FR3 | The divider branch currently ignores `icon`; this draft MUST NOT convert that observation into a stable restriction or a requirement to render the icon.                                | Current source versus unrestricted released prop description               | Human API decision required; see OQ1                    |
...
| Default with icon     | Leading content precedes visible content.                                | Caller-provided artwork and content.      |
| Divider               | A named horizontal separator presents the supplied content.              | Label content, theme, and root styling.   |
...
| Narrow/coarse pointer | The noninteractive row remains visible without horizontal page overflow. | Theme and supported concise content.      |
...
- **AR2 - Divider relationship.** The divider branch delegates a horizontal
  `separator` whose accessible name references the visible label.
...
| Leading icon     | Reinforces default-branch content without replacing its visible text. | Current source and released consumer docs; divider unsettled | Supporting     | FR1, FR3, AR3      |
...
| ------------------------- | --------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------- |
| FR1, AR1, AR3             | `ChatSystemMessage.test.tsx`, scoped axe, and exact-head Chromium receipts                                      | Default, default with icon, neutral light/dark                                                          | Missing content, status role, leading content, or browser-visible subject fails focused or browser evidence.                                       | `audit:ChatSystemMessage/accessibility` |
...
| FR4                       | Existing focused root-passthrough tests, source inspection, type checks, and target reflection                  | Default and divider roots                                                                               | A supported root input disappears or overrides the owned role/state and fails focused tests or review.                                             | `audit:ChatSystemMessage/api`           |
...
| Browser matrix            | `ChatSystemMessage.a11y.chromium.spec.ts` exact-head 10-sensor receipts                                         | Default, icon, divider, unresolved divider/icon observation, 320px/coarse pointer, light/dark, D7 pairs | Stale build, wrong story, incorrect mode, missing semantics, overflow, failed contrast, blank crop, or page error fails closed.                    | `audit:ChatSystemMessage/visual`        |
...
None. This draft records released observable behavior and audit evidence. It
introduces no public API, default, compatibility promise, ownership transfer, or
subjective visual decision.

## Open questions

- **OQ1 - Should `icon` render before the label in the `divider` variant, or should the public contract explicitly restrict `icon` to the default variant?** (`human-api`)
