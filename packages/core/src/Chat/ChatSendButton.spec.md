...
| Behavior                | Send is disabled from explicit or composer readiness, stop remains enabled, state-specific actions run before a composed generic click handler, and icon resolution follows the applicable current slot authority.                     |
...
| Compatibility/readiness | This draft records current or remediated observable behavior. It adds no public prop, default, target, compatibility promise, or migration. One icon-slot conformance gap remains owned by current `spec:AST-032`.                     |
| Review checks           | Reject lost send/stop routing, consumer events that replace the state action, a disabled stop action, unlabelled icon-only output, dropped root inputs, or a send fallback that skips current component-slot precedence.               |
| Governing rules         | `spec:AST-032/FR2-FR3`; `architecture:public-component-api/INV5-INV8`; `architecture:interaction-modality`; `architecture:component-test-sufficiency`; `spec:AST-020`; `spec:AST-029`.                                                 |
...
| Send icon    | explicit content; component slot; shared fallback              | Selects artwork for the send state.                                                    | Send state.                               | `sendIcon`, then `chat-send-button-send`, then `arrowUp`.   | `spec:AST-032` and caller           | Current required surface   | A component-slot `null` intentionally suppresses the icon under the current slot contract.  |
...
| FR2 | The stop state MUST render a secondary icon-only Button named by the stop translation, remain enabled regardless of send readiness, and invoke explicit or context stop behavior.                                 | Current implementation, docs, and tests                                                                                | Verified current behavior; the missing-handler public boundary remains OQ1               |
| FR3 | Send icon resolution MUST prefer `sendIcon`, then the `chat-send-button-send` component slot, then `arrowUp`. A component-slot `null` MUST suppress the icon.                                                     | Current `spec:AST-032/FR2-FR3` and `architecture:icon-resolution-and-component-slots`                                  | Settled current authority; the component-slot stage is a pre-existing implementation gap |
...
- **AV2 — Artwork.** Explicit send and stop content and theme-resolved send artwork
  may vary within the current icon-resolution contract.
...
- **ORD4 — Resolve artwork.** Apply FR3 or FR4 for the selected state.
...
| Send artwork     | Upward send glyph unless explicit or theme component artwork wins. | `spec:AST-032`                                               | Action cue               | FR3                |
...
- `architecture:icon-resolution-and-component-slots` owns component-icon
  precedence; `spec:AST-032` applies that model to the send state.
...
| ------------------------- | -------------------------------------------------------------------- | ---------------------------------------------- | --------------------------------------------------------------------------------------------- | ------------------------------------ |
| FR1, FR2, AR1-AR4         | `ChatSendButton.test.tsx`, Storybook a11y owner routes, focused CI   | Enabled/disabled send, stop, keyboard/pointer  | Label, disabled, native activation, or state-routing drift fails focused or browser evidence. | `audit:ChatSendButton/accessibility` |
| FR3                       | `spec:AST-032` focused implementation tests in its owning change     | Explicit, themed, null, and fallback send icon | Skipping precedence or mishandling null fails the current authority's focused tests.          | `audit:ChatSendButton/theming`       |
...
| FR5, FR6                  | Red-before-green callback composition tests, types, lint, and source | Send, stop, root input composition             | Consumer events replacing state actions, lost props, or lost ref/style composition fails.     | `audit:ChatSendButton/api`           |
| FR7                       | `themingTargets.test.ts`, probe checks, and source inspection        | Send and stop themes                           | Target removal, movement off paint, or undocumented target drift fails tests or inspection.   | `audit:ChatSendButton/theming`       |
| RTL relation              | RTL shared-fixture route plus verified-N/A source receipt            | Send and stop                                  | Directional artwork, physical positioning, or transform invalidates the checked-in receipt.   | `audit:ChatSendButton/i18n-rtl`      |
...
and one current-authority conformance gap. It introduces no component-local public
API, default, compatibility, ownership, or subjective visual decision.

## Open questions

- **OQ1 — Should `isStopShown` without an explicit or context `onStop` remain an
  enabled no-op, become disabled, or be rejected?** (`human-api`) Current source and
  draft ChatComposer context do not settle this public invalid-state policy, so this
  audit records the boundary without changing it.
