...
| Root surface   | `BaseProps<HTMLDivElement>`, `ref`, `xstyle`, `className`, `style` | Extends the metadata row when it exists                           | Only when an item renders          | No element for empty content | `architecture:public-component-api` | Released      | Component target and consumer styles combine                                                         |
