...
function measureAverage(iterations: number, callback: () => number): number {
  const start = performance.now();
...
function overheadRatio(
  ...
  // Warm both paths before sampling, then alternate their order so startup and
  // runner drift cannot consistently favor either side of the ratio.
  ...
  for (let round = 0; round < 9; round++) {
    ...
    let dispatch: number;
    if (round % 2 === 0) {
      baseline = measureAverage(20, () => parseMarkdown(source).length);
      dispatch = measureAverage(20, () => visitBlocks(blocks, text));
    } else {
      dispatch = measureAverage(20, () => visitBlocks(blocks, text));
      baseline = measureAverage(20, () => parseMarkdown(source).length);
    }
    ...
    ratios.push((baseline + dispatch) / baseline);
  }
  void benchmarkSink;
  return {
    baseline: median(baselines),
    dispatch: median(dispatches),
    ratio: median(ratios),
  };
}
