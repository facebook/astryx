...
| FR26 | On the default path each table column has a content-derived width floor and never shrinks below it: the larger of its min-content — the longest unbreakable token it holds, so identifiers and URLs never break mid-word — and a readable minimum that lets the column's prose wrap to a few lines rather than one word per line, scaled by the column's own content length and bounded so a table of short columns still fits its container. When the floors exceed the container the table grows past it and Table's Scroll region scrolls; columns do not squash to fit. Markdown's default Table part sizes its own header and body cells over Table's wrap-mode defaults — no `max-width`, wrapping with `overflow-wrap: break-word`, and neither `overflow-wrap: anywhere` nor its `word-break: break-word` equivalent — and adds no prop, option, or override slot to do so. Exact widths are implementation tuning. |
...
| FR26                   | `Markdown.test.tsx`, browser layout test, Storybook                           | Long unbroken token; many short columns in a narrow container; prose columns in a narrow container; fitting table                      | A column breaks a token mid-word, wraps prose one word per line, or squashes below its floor; a fitting table starts to overflow; Table's released wrap mode changes for data-driven tables. |
