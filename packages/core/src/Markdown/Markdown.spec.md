...
owners: [cixzhang]
review_triggers: [api, theming, layout]
verified_by:
  [
...
remain unchanged when plugins and math are absent. A table's columns are never
narrower than their content can reasonably occupy; a table wider than its
container scrolls rather than squashing.
...
- Compatibility class: additive, opt-in public API; existing parser nodes, DOM,
  ...
  passes the matching explicit parser option. The column width floor is a
  default-path layout change: columns may become wider than today's fixed
  buckets, never narrower, and a table that now squashes may instead scroll.
...
- The column width floor of its default Table part: Markdown sizes its own
  header and body cells over Table's wrap-mode defaults so a column's minimum
...
- Nested anatomy or targets owned by CodeBlock, Blockquote, List, CheckboxList,
  or Table, including Table's Scroll region behavior under `spec:AST-025` and
  Table's released wrap-mode cell sizing for data-driven tables.
...
| FR26 | On the default path each table column has a content-derived width floor and never shrinks below it: the larger of its min-content — the longest unbreakable token it holds, so identifiers and URLs never break mid-word — and a readable minimum that lets the column's prose wrap to a few lines rather than one word per line, scaled by the column's own content length and bounded so a table of short columns still fits its container. When the floors exceed the container the table grows past it and Table's Scroll region scrolls; columns do not squash to fit. Markdown's default Table part sizes its own header and body cells over Table's wrap-mode defaults — no `max-width`, wrapping with `overflow-wrap: break-word`, and neither `overflow-wrap: anywhere` nor its `word-break: break-word` equivalent — and adds no prop, option, or override slot to do so. Header cells follow the same rule and never truncate: no ellipsis and no `max-width` clamp; a header label reads on one line up to a bounded width and wraps to a few lines past it, and its length counts toward the column floor alongside the body cells. Exact widths are implementation tuning. |
...
| Wide table             | Every column keeps its content-derived floor; the table overflows into Table's Scroll region instead of squashing columns, and the scroll wrapper keeps its released semantics. | Column count, alignment, cell content, and density.                                             |
...
- The column width floor is CSS on the cells; no measurement, observer, or
  per-table state is added for it.
...
| Table            | Presents parsed rows and columns in a keyboard-scrollable block wrapper.           | Current source and public docs | Prominent      | FR2, FR26          |
...
- `spec:AST-025` and `component:Table/DEC-1` own the Scroll region's effective
  ...
  block's own inline overflow never engages. FR26 relies on that region to take
  the width the column floors produce and adds no scroller of its own.
...
| FR26                   | `Markdown.test.tsx`, browser layout test, Storybook                           | Long unbroken token; many short columns in a narrow container; prose columns in a narrow container; long header over a short body; fitting table | A column breaks a token mid-word, a header truncates, wraps prose one word per line, or squashes below its floor; a fitting table starts to overflow; Table's released wrap mode changes for data-driven tables. |
...
### DEC-6 — Markdown table columns keep a content-derived floor

**Reference:** `component:Markdown/DEC-6`
**Decider:** `cixzhang`, `2026-09-25`

Table's wrap mode sizes cells with `max-width: 0` and `word-break: break-word`,
which zeroes each cell's min-content contribution so automatic layout divides
the container equally among the columns. Inside a narrow reading column a
six-column Markdown table then squashes every cell to a few characters, breaks
identifiers mid-word, and never overflows, even though a Scroll region is
waiting to take the width. Markdown's released per-column width bucket did not
prevent this: a fixed bucket under-sizes any column whose content exceeds it,
and a bare min-content floor would not either, because prose would wrap one
word per line.

Markdown therefore owns the floor for its own default Table part: the larger of
the longest unbreakable token and a readable minimum derived from the column's
content, applied as cell styles over Table's wrap-mode defaults. The table
grows to the sum of its floors and Table's existing Scroll region scrolls it.
This adds no prop, option, or override slot — a reader's table should read
without anyone choosing anything — and leaves Table's released wrap mode for
data-driven tables untouched.

Header cells are part of the same rule. Table's header cell truncates with an
ellipsis in every mode, which is right for a data grid with a resizable column
and wrong for prose a reader cannot widen: a Markdown header reads in full, on
one line up to a bounded width and wrapped past it, and its length counts toward
the column floor so a short-bodied column is still as wide as its label.

Rejected: a fixed width bucket as the only floor; a bare min-content floor;
changing Table's released wrap-mode sizing; a `components.table` override slot;
an overflow indicator, whole-table viewer, or any new Markdown prop or theme
target.
...
- **OQ2 — Should Markdown's `markdown-table` block stop being a second
  ...
  stops both named "Table". Released DOM; left for a follow-up. (`checkable`)
