...
separately add native fragment permalinks beside top-level default headings or
...
- Compatibility class: additive, opt-in runtime API; existing parser values, DOM,
  ...
  `hasHeadingPermalinks`, enables `footnotes="github"`, supplies `plugins`, supplies
  `components.math`, or passes the matching explicit parser option. Released parser
  result unions remain narrow by option. The shared canonical AST unions add the two
  Core footnote node kinds, so exhaustive canonical-AST TypeScript consumers add those
  cases when upgrading.
- Controlled/uncontrolled behavior: not applicable
- Migration decision: `module:Markdown/footnotes` compatibility section
...
- Default Heading, Paragraph, List, Code block, Blockquote, Table, Divider, and
  Image block presentation and the eight established block targets documented below.
- The optional generated Footnote section and its `markdown-footnotes` target when
  Core footnotes are enabled and referenced.
...
- Applying separately opted-in block footnote parsing, first-reference numbering,
  document-global fragment allocation, a labelled ordered section, and one native
  backlink per rendered reference under `module:Markdown/footnotes`.
...
through a compatibility projection; explicit footnote options select separate recursive
result families. The public canonical unions include Core's `footnoteReference` and
`footnoteDefinition` kinds even though default parse values do not emit them, so
exhaustive canonical consumers account for both. Canonical image nodes preserve an optional
...
`prepareMarkdownDocument()` is the server-safe one-parse composition boundary: it
returns an opaque prepared document containing the canonical root and matching
readonly Outline items. A block `Markdown` render may receive that object through
`document` instead of `children`; parsing, transforms, heading projection, and any
enabled Core-footnote projection are then reused rather than repeated. Prepared
documents are not an RSC-serialization format, and inline or smoothed streaming
rendering continues to use the existing parser paths.
...
| FR23 | Markdown owns an explicit supported dialect, not full CommonMark or GFM. Compatible adjacent ordered or unordered items remain one list regardless of task markers; each item preserves checked or ordinary semantics at every level. Blank-separated fenced blocks stay in an item when indented to its marker content. The default grammar retains task lists and tables; `autolink: 'gfm'` adds only documented autolinks and no other syntax.                                                                                                                                                                                                                                                                                                                                                                                              |
...
name containing that text. Enabled Core footnotes use a localized semantic section,
an ordered list, numbered native-fragment reference Links, and one distinctly named
...
compose the existing Link target without adding a target. Core footnotes add the
section target only when referenced definitions render; their reference and backlink
controls retain Link's target.
...
| FR31                   | footnote parser, incremental, protocol, projection, render, public, Remark, Outline, locale, and Storybook tests | omitted/enabled, grammar/protected contexts, repeated refs, collisions, transforms, prepared/streamed output, router links, theme target | Source disappears, identity drifts, a plugin forges semantics, navigation loses a counterpart, inline accepts the mode, or Remark accepts the nodes.                                              |
| Public syntax/types    | `Markdown.public.test.ts`, parser/prepared public tests, core typecheck, and `Markdown.doc.mjs`                  | Legacy exhaustive switches, explicit footnote/math families, canonical footnote cases, inferred extension-node unions                    | A released default result union widens, an enabled union loses nodes, canonical cases are unexported, or docs drift from declarations.                                                            |
...
Focused tests continue to pin the nine established target names and default block
placement. Math intentionally adds no target and no default anatomy. Heading
permalinks compose the existing Link target. Enabled Core footnotes add the optional
`markdown-footnotes` section target while their links retain Link ownership.
