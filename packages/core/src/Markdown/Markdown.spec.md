...
identity. Transforms receive a stable public MDAST-aligned tree rather than the
private parser tree; `MarkdownAstNodeMap` and `visitMarkdownNodes` provide strict
node-kind narrowing. Transforms return validated replacement roots without entering
parse identity. Every extension node introduced
...
| FR13 | Plugin-enabled parsing follows built-in lexical shields and ordered syntax claims, then translates the private parser tree into one stable, strictly typed, MDAST-aligned public transform root. `MarkdownAstNodeMap` and `visitMarkdownNodes` narrow callbacks by node kind. Every returned root is finite, acyclic, representable, validated, and frozen before later plugins or rendering observe it.                                                                                                                  |
