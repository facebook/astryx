...
that option with `createIncrementalState<true>()`. Default calls and values
...
| FR11 | Streaming converges to the same nodes as a full parse at every chunk boundary, including display math nested in ordinary lists, task lists, blockquotes, and their supported combinations, with LF or CRLF and with or without source ranges. Incomplete math is withheld only while its owning container remains open. Math-enabled incremental calls require `IncrementalState<true>`, so the cache and returned union share one contract. |
...
`MathParseOptions`; incremental callers also create `IncrementalState<true>` so
the cache and result expose the same math-enabled node union.
