...
export const docsDense = {
  description: 'single on/off toggle for settings, terms, + opt-in choices; use CheckboxList for groups instead',
  usage: {
    bestPractices: [
      { guidance: true, description: 'Always provide visible label; use isLabelHidden only when surrounding context makes it obvious' },
      { guidance: true, description: 'Add description for choices needing extra context' },
      { guidance: true, description: 'Use indeterminate for "select all" when only some items selected' },
      { guidance: false, description: 'checkbox for mutually exclusive choices — use RadioList when only one can be selected' },
      { guidance: false, description: 'checkbox for immediate-effect actions — use toggle switch or button instead' },
    ],
  },
  propDescriptions: {
    label: 'label text; always rendered for a11y **(required)**',
    isLabelHidden: 'visually hide label; still accessible to screen readers',
    description: 'text below label w/ additional context',
    value: 'checked, unchecked, or indeterminate **(required)**',
    onChange: 'callback on state change',
    changeAction: 'async action; fires after onChange if not prevented; shows spinner while pending',
    isLoading: 'shows spinner + prevents interaction',
    isDisabled: 'disable checkbox',
    isOptional: 'mark field optional; mutually exclusive w/ isRequired',
    isRequired: 'mark field required; mutually exclusive w/ isOptional',
    size: 'sm (compact) / md (default)',
    onFocus: 'callback on focus',
    onBlur: 'callback on blur',
    labelIcon: 'icon before label; see `npx xds docs icons` for valid names',
    status: 'error/warning/success w/ message; sets aria-invalid on error',
  },
};
