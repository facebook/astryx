...
export const docsDense = {
  description: 'text input w/ calendar popover for typing or picking a date',
  usage: {
    description: 'Type or pick date from calendar popover. Use for scheduling, deadlines, booking, or any form field needing a calendar date.',
    bestPractices: [
      { guidance: true, description: 'Clear labels + descriptions so users understand expected date.' },
      { guidance: true, description: 'Use min, max + dateConstraints to restrict selectable dates to valid ranges.' },
      { guidance: true, description: 'Use hasClear when date is optional so user can easily reset.' },
      { guidance: true, description: 'Show loading state w/ changeAction when date triggers server-side save.' },
      { guidance: false, description: 'Use DateInput for free-form text not representing a calendar date.' },
      { guidance: false, description: 'Hide label w/o surrounding context making field purpose obvious.' },
      { guidance: false, description: 'Rely on calendar alone — text input lets users type dates directly; faster for known dates.' },
    ],
  },
  propDescriptions: {
    ...
    onChange: 'callback when selected date changes',
    ...
    hasClear: 'shows × button when date set; clears value + returns focus to input',
