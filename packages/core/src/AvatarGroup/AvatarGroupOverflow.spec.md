...
| FR6 | An interactive indicator inside AvatarGroup currently carries the group's item marker and participates in AvatarGroup's existing roving-focus sequence; a static indicator does not become a focusable item.                                               | Current source and AvatarGroup interaction tests            | Verified composition; AvatarGroup retains focus-system ownership |
...
| --------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------ | --------------------------------------------------------------------------------- |
| Static default count  | One non-focusable span presents the translated count label and visible `+N` content.                                                             | Count, group size, group shape, direction, and consumer styles may vary.          |
| Static custom content | The span retains the count-derived accessible label while caller content replaces the visible default.                                           | Caller owns the custom React content.                                             |
...
- **AR2 — Current native activation.** The interactive branch is a native
  `button` with `type="button"` and the shared focus-visible treatment; the static
  branch remains a span.
- **AR3 — Current group participation.** The interactive branch carries the
  current AvatarGroup item marker so AvatarGroup's existing focus owner may include
  it. This draft does not copy or change AvatarGroup's focus algorithm.
...
| ------------------------- | ------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------ | ----------------------------------------- |
| FR1, FR2, AR1, AR2        | `AvatarGroupOverflow.test.tsx` render, callback, content, count, ref, and locale suites           | Static, clickable, custom content, zero, negative, large, localized, and standalone | Changing the root branch, callback, normalized count, visible fallback, accessible label, or ref target fails focused public-interface assertions.     | `audit:AvatarGroupOverflow/behavior`      |
...
| FR6, AR3                  | `AvatarGroup.test.tsx` overflow-button roving-focus suite                                         | Interactive overflow after an interactive Avatar                                    | Removing the marker or group participation fails the public keyboard-composition assertion.                                                            | `audit:AvatarGroupOverflow/accessibility` |
