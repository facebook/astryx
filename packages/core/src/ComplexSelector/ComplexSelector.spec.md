...
| FR5 | With `hasClear`, a clear button renders between the loading spinner and the Indicator icon while `triggerLabel` is set and the field is enabled; activating it calls `onClear` (else `onChange(undefined)`) and does not open the Popup. | Selector's `hasClear` placement | Added with `hasClear`; mirrors Selector            |
