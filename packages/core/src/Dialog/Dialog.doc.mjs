...
export const docsDense = {
  description: 'modal overlay blocking page interaction until user responds',
  usage: {
    description: 'modal overlay blocking page interaction; use for delete confirmations, edit forms, terms acceptance, or decisions that must not be skipped. For state-free usage, use useXDSImperativeDialog hook — call dialog.show(content) + render dialog.element in JSX tree.',
    bestPractices: [
      { guidance: true, description: 'Pick right purpose: info=dismissable; form=no backdrop dismiss after interaction; required=user must respond.' },
      { guidance: true, description: 'Include clear title in header so users immediately understand what dialog asks.' },
      { guidance: true, description: 'Use purpose="form" for dialogs w/ inputs so user can\'t accidentally lose data by clicking backdrop.' },
      { guidance: true, description: 'Keep dialogs focused on single task — if content outgrows fit, consider full page instead.' },
      { guidance: false, description: 'Use dialog for simple messages that could be inline or toast.' },
      { guidance: false, description: 'Nest dialogs inside other dialogs — restructure into steps within single dialog instead.' },
      { guidance: false, description: 'Use fullscreen variant for simple confirmations — meant for complex content like editors/long forms.' },
    ],
  },
  components: [
    {
      ...
      propDescriptions: {
        ...
        onOpenChange: 'callback on visibility change **(required)**',
        width: 'dialog width (px or CSS value); defaults to 400',
        maxHeight: 'max dialog height; defaults to \'75vh\'',
        position: 'static position; centered when omitted',
        variant: 'standard or fullscreen (fills viewport); defaults to standard',
        purpose: 'dismissal behavior: required=no Escape + no backdrop click; form=no backdrop click after interaction; info=both allowed; defaults to info',
        isInline: 'renders content inline w/o <dialog>, backdrop, or modal behavior; for docs previews/showcases only; defaults to false',
      },
    },
    {
      name: 'XDSDialogHeader',
      ...
      propDescriptions: {
        ...
        hasDivider: 'bottom edge border; defaults to true',
      },
    },
    {
      name: 'useXDSImperativeDialog',
      displayName: 'useXDSImperativeDialog',
      description: 'hook for showing dialog w/o managing open state; call dialog.show(content, options) to open + dialog.hide() to close; render dialog.element in JSX tree',
      propDescriptions: {
        show: 'show dialog w/ given content; options = XDSDialog props minus isOpen/onOpenChange/children',
        hide: 'hide dialog',
        isOpen: 'whether dialog currently open',
