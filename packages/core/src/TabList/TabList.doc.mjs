...
export const docs = {
  ...
  props: [
    ...
    },
    {
      name: 'isFullBleed',
      type: 'boolean',
      description: "Makes the tab strip escape its parent's container padding, extending to the container's content edges (cancels the nearest padded Layout container's --container-padding-* custom properties with negative margins). Docks a tab bar flush with a header's content edges so a hasDivider underline meets the header divider, replacing the negative-margin CSS that case otherwise needs. Matches Divider's isFullBleed. Inline edges always bleed; block edges bleed when the TabList is the first/last child.",
      default: 'false',
    },
    {
      name: 'role',
  ...
  usage: {
    ...
    bestPractices: [
      ...
      { guidance: true, description: 'Set isFullBleed to dock a tab bar inside a padded LayoutHeader, Card, or Section so the strip meets the container\'s content edges, instead of reaching for negative-margin CSS.' },
