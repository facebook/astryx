  ...
  sections: [
    ...
    },
    {
      ...
      content: [
        {
          type: 'prose',
          text: 'adaptations = ordered {when,value} rules over width/pointer/contrast/motion. widthBreakpoints fixed sm|md|lg|xl|2xl defaults 640|768|1024|1280|1536; map alone emits no CSS. width.from inclusive, width.below exclusive; condition fields AND. root first, then matching rules in authored order (later writes win), then onDark/onLight. rules can write typography/color/radius/motion/tokens/localTokens/components; local names belong on root. A generated Core contract validates all current finite visual-prop domains: closed axes accept built-ins only; custom augmentation-wired values must exist on root before rules use them. Older Core packages without the contract retain the legacy boundary. Co-matching token/localToken writes are validated together; any reachable var() cycle fails. extends inherits breakpoints + ordered rules, appends child rules, re-resolves against child axes. CSS-only; use built themes for SSR first paint.',
        },
        null,
        null,
      ],
