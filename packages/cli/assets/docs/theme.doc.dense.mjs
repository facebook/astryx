...
export const docsDense = {
  ...
  sections: [
    ...
    { section: 'Building Themes for Production', title: 'Build for Production', content: [{ type: 'prose', text: "`astryx theme build` outputs `.css`, `.js` (`__built:true`), and `.d.ts`. If `icons` directly references an unaliased named import (`import {oceanIcons} from './icons'`; `icons: oceanIcons`), generated JS imports a companion module; caller compiles it separately." }, null, null, null, { type: 'prose', text: '`--icons-specifier` only rewrites that import, resolved from generated JS; emit a loadable companion module there.' }, null, { type: 'prose', text: 'replace `lucide-react` with the registry dependency, or omit `--external` to bundle it.' }] },
