...
describe('findSourceFiles — scan boundaries (symlink + build dirs)', () => {
  ...
  it('blocks generated files only when a codemod would modify them', async () => {
    const srcDir = path.join(tmpDir, 'src');
    fs.mkdirSync(srcDir);
    const generated = path.join(srcDir, 'theme.css');
    const autoGenerated = path.join(srcDir, 'tokens.css');
    const ordinary = path.join(srcDir, 'app.css');
    const mentionsGeneratedLater = path.join(srcDir, 'documented.css');
    const documentsGeneratedMarker = path.join(srcDir, 'marker-doc.css');
    const unchangedGenerated = path.join(srcDir, 'unchanged.css');
    fs.writeFileSync(
      generated,
    ...
    fs.writeFileSync(ordinary, '.foo { color: red; }\n');
    fs.writeFileSync(
      mentionsGeneratedLater,
      '#app { color: red; }\n/* @generated is documented here. */\n',
    );
    fs.writeFileSync(
      documentsGeneratedMarker,
      '/* This source documents the @generated marker. */\n.foo { color: red; }\n',
    );
    fs.writeFileSync(
      unchangedGenerated,
    ...
    const manifests = [
      {
        ...
        transforms: [
          {
            name: 'replace-red',
            meta: {title: 'replace red', fileExtensions: ['.css']},
            transform: file => file.source.replace('red', 'green'),
          },
        ],
      },
    ];
    ...
    expect(result.generatedFilesBlocked).toEqual([
      'src/theme.css',
      'src/tokens.css',
    ]);
      ...
      expect.arrayContaining([
        expect.objectContaining({
          ...
          error: expect.stringContaining(
            'generated file would have been modified',
          ),
        }),
        expect.objectContaining({file: 'src/tokens.css'}),
      ]),
    );
    expect(result.totalFilesChanged).toBe(3);
    expect(fs.readFileSync(generated, 'utf-8')).toContain('color: red');
    expect(fs.readFileSync(autoGenerated, 'utf-8')).toContain('color: red');
    expect(fs.readFileSync(ordinary, 'utf-8')).toContain('color: green');
    expect(fs.readFileSync(mentionsGeneratedLater, 'utf-8')).toContain(
      'color: green',
    );
    expect(fs.readFileSync(documentsGeneratedMarker, 'utf-8')).toContain(
      'color: green',
    );
    expect(fs.readFileSync(unchangedGenerated, 'utf-8')).toContain(
      'color: blue',
    );
  });
