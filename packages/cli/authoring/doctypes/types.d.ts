 ...
 *
 * Every template directory under `packages/cli/assets/templates/` has a
...
// ---------------------------------------------------------------------------
// Authoring input vocabulary — the shapes an integration author writes for a
// `.doc.*` / `.template.*` file (relocated from `@astryxdesign/core/authoring`).
// These are the loose, author-facing surfaces; the CLI validates a loaded value
// against them via `parseDoc` at the load boundary. No factory is needed — an
// author writes a plain object and stamps `type` directly.
// ---------------------------------------------------------------------------
...
/** A generic reference/topic doc — just the shared base. */
...
/** Input for a page template doc (no `type` field). */
export type AstryxPageTemplateInput = AstryxTemplateInput;
/** Input for a block template doc (no `type` field). */
