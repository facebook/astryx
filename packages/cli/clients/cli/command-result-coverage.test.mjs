...
/**
 * Commands with no action of their own — a group that only prints its
 * subcommand list, with no run to report on.
 *
 * Currently empty: every shipped command has an action. Pinned as a SET rather
 * than skipped silently — if a command joins this list, that is a real change
 * to what the CLI does and it should be read, not absorbed. (`theme` is
 * deliberately NOT here: it HAS an action — the one that rejects an unknown
 * subcommand — so it goes through the converter like any other. Its bare form
 * prints help and never returns from that action, which the recorder covers
 * where help is recorded, not here.)
 */
