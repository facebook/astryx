...
| `theme.build`               | A theme build receipt: name, token- and component-override counts, output size, the written outputs {css, js, dts, optional variantsDts, and opt-in palette artifacts when applicable}, plus validation warnings and notices.                                                       |
| `theme.build.check`         | The --check receipt: theme name, an upToDate flag, the stale outputs (each {path, reason: missing \| outdated \| obsolete}), and the full list of checked paths. Writes nothing.                                                                                                    |
