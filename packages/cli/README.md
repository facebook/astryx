...
| `cdn`                  | Use Astryx from a CDN, with no build step                                     |
...
| `cdn.template`              | A write receipt for the no-build-step CDN starter page: the path (relative to cwd), the Astryx version every CDN URL was pinned to, whether it was written, and the reason it was not — `exists` when a file was already there, which is a success. |
