...
| `template.show`                   | The resolved template's raw source plus its description, kind, the components it composes, and the resolvable subset of those components (every name in resolvableComponents resolves through `astryx component <Name>`).                                                                                                                                                                                                                  |
| `template.skeleton`               | A layout skeleton (structural tags with spatial annotations) plus the template's description, the components it composes, and the resolvable subset of those components (every name in resolvableComponents resolves through `astryx component <Name>`).                                                                                                                                                                                   |
