...
### Scaffold w/ CLI (recommended)

\`\`\`bash
npx xds theme
\`\`\`

Interactive wizard generating \`.stylex.ts\` theme file w/ your brand colors.

### Manual creation

Theme = object w/ \`name\` + \`styles\`. Only override token groups differing from defaults; omitted groups auto-use \`defineVars\` defaults from \`@xds/core\`.
...
Omitted groups use \`defineVars\` defaults from \`tokens.stylex.ts\`.

### ⚠️ Static Analysis Constraint

StyleX Babel plugin requires **all values passed to \`stylex.createTheme()\` be inline object literals**, not variable refs, spread expressions, or function returns. Compile-time constraint.
...
Only create \`stylex.createTheme()\` for token groups you want to change. Example, theme only changes colors+radius:
...
Then \`mode="system"\` (default) on XDSTheme follows OS preference.
...
Set \`mode\` explicitly, use single values (not \`light-dark()\`):
...
\`XDSTheme\` uses \`display: contents\`, doesn't create visual box.
Apply theme background color to wrapper element via StyleX:
...
Themes can override individual component styles via \`components\` field.

### How it works

1. Components register themeable properties via module augmentation:
...
2. Theme provides StyleX overrides:
...
Run \`npx xds component metadata <Name>\` to see component's themeable slots.

### \`as Theme['components']\` cast

\`components\` field requires type assertion because \`ComponentStyles\` augmented by individual components via \`declare module\`. Theme package doesn't import those components, so augmentations aren't visible at compile time. Cast safe; types enforced at consumer site.
...
Read-only. To change theme/mode, manage state at app level + pass to \`<XDSTheme>\`.
`,
};
