...
describe('scoreCandidate() — altNames run the name ladder', () => {
  it('scores an exact authored-name hit as an exact name match', () => {
    // A page is addressed by its directory but authored with a human name;
    // both are identity, so the authored name must land the same 100 the
    // directory name would.
    const hit = scoreCandidate('landing page', {
      name: 'marketing-landing',
      altNames: ['Landing Page'],
    });
    expect(hit).toMatchObject({score: 100, reason: 'exact name'});
  });

  it('keeps the stronger signal when both names are present', () => {
    const hit = scoreCandidate('table', {name: 'table', altNames: ['Data Grid']});
    expect(hit).toMatchObject({score: 100, reason: 'exact name'});
  });

  it('changes nothing for candidates without altNames', () => {
    expect(scoreCandidate('button', {name: 'Button'})).toMatchObject({
      score: 100,
      reason: 'exact name',
    });
    expect(scoreCandidate('buttn', {name: 'Button'})?.score).toBe(80);
  });

  it('finds a page by its authored name end-to-end', async () => {
    const r = await search('landing page', {cwd, type: 'template', limit: 40});
    expect(r.data.results[0]?.name).toBe('marketing-landing');
  }, SLOW);
});
