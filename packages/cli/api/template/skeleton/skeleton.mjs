 ...
 * @param {Set<string>|((name: string) => boolean)|null} [isResolvable] exact resolvability oracle (a prebuilt name set or a per-name predicate), or null to skip filtering
 * @returns {import('../template.type.mjs').TemplateSkeletonResponse}
 */
export function templateSkeleton(match, templates, isResolvable = null) {
  if (!match) {
  ...
  return {
    ...
    data: {
      ...
      // Released meaning, preserved for compatibility: every component the
      // template composes, whether or not it resolves.
      components: extractComponents(match.filePath, null),
      // The resolvable subset: every name here resolves through
      // `astryx component <Name>`.
      resolvableComponents: extractComponents(match.filePath, isResolvable),
