...
describe('template --skeleton component extraction (prefix-agnostic)', () => {
  ...
  it('extracts components and a skeleton from a bare-named template', async () => {
    ...
    // The released components list is preserved whole; the resolvable
    // subset carries the exact-resolution contract.
    expect(Array.isArray(result.data.resolvableComponents)).toBe(true);
    expect(result.data.resolvableComponents).toContain('Card');
    expect(result.data.resolvableComponents).toContain('TextInput');
...
describe('template --skeleton advertises only resolvable components', () => {
  ...
  it('source/parent-doc resolvable names are kept, not omitted (#4677)', async () => {
    ...
    // in resolvableComponents. `AppShellMobileContext` has a source file
    // but no doc of its own (the parent AppShell.doc.mjs covers it);
    // `NavHeadingMenuItem` likewise resolves through the parent NavMenu doc.
    // A filename-heuristic filter dropped both; the exact oracle keeps them.
    const skeleton = await template('MobileNavToggleBasic', {skeleton: true});
    expect(skeleton.data.resolvableComponents).toContain(
      'AppShellMobileContext',
    );
    // The released components list still carries every composed name.
    expect(skeleton.data.components.length).toBeGreaterThan(0);
    const show = await template('NavHeadingMenuShowcase', {show: true});
    expect(show.data.resolvableComponents).toContain('NavHeadingMenuItem');
    ...
    // surface as Timeline in the resolvable list (the released list keeps
    // upstream's base-name normalization and still reports Timeline).
    const detail = await template('detail-page', {skeleton: true});
    expect(detail.data.resolvableComponents).not.toContain('Timeline');
    expect(detail.data.resolvableComponents).not.toContain('TimelineSection');
