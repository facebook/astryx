...
export const doc = {
  ...
  description:
    ...
    'package.json without a config entry, project-wide integration contribution ' +
    'issues (with every structured issue preserved), agent docs, core peer ' +
  ...
  returns: [
    {
      type: 'doctor',
      description:
        'The diagnostic report: `data.checks`, each with a stable id, label, `status` (`pass` | `warn` | `fail` | `info`), a one-line message, a `fix` when the status is not `pass`, and optional structured integration `issues`; plus `data.summary` with counts per status.',
    },
  ],
  examples: [
