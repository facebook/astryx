...
describe('themeBuild() — contrast-pair warnings in the receipt', () => {
  it('warns when a scale-config theme overrides an accent but not its on-colour', async () => {
    ...
    expect(warnings[0]).toMatch(
      /you set --color-accent but left --color-on-accent/,
    );
    // Not a notice: this one is the theme's to fix.
    expect(result?.data.notices ?? []).not.toContain(warnings[0]);
  });

  it('warns when a shared-parser named color creates a failing pair', async () => {
    const result = await build(
      'named.mjs',
      `export default {
         name: 'named',
         tokens: {'--color-error': 'white'},
       };\n`,
    );

    expect(contrastWarnings(result)).toEqual([
      expect.stringContaining('label on the error fill is 1:1'),
      expect.stringContaining('label on the error fill is 1:1'),
    ]);
  });

  it('returns the same warning from check mode', async () => {
    await build(
      'checked.mjs',
      `export default {
         name: 'checked',
    ...
    expect(contrastWarnings(result).length).toBeGreaterThan(0);
    expect(contrastWarnings(result)[0]).toMatch(/label on the error fill/);
  });
