 ...
 * @param {{digest: string|null, count: number, complete: boolean}} [inputs] -
 *   Digest over every local file this theme was compiled from. Recorded so
 *   `doctor` can verify freshness by reading and hashing those files, instead
 *   of recompiling — which would mean executing the theme's whole import graph.
 *   When the input set could not be walked completely, the line says so rather
 *   than carrying a digest that omits an input.
 */
function generatedHeader(sourceFile, lang = 'js', command, versions, inputs) {
  const body = [
  ...
  if (inputs) {
    body.push(
      inputs.digest
        ? `Inputs: sha256-${inputs.digest} (${inputs.count} file${inputs.count === 1 ? '' : 's'})`
        : `Inputs: unverifiable`,
    );
  }
  if (lang === 'css') {
...
export async function themeBuild(
  ...
  // A digest over every local file this theme is compiled from, so freshness
  // can later be checked by reading those files rather than by recompiling.
  // The entry's own mtime cannot answer that question: a theme imports its
  // tokens, and editing a token leaves the entry untouched.
  const inputs = themeInputsDigest(filePath);
  ...
  const variantContent = variantDecl
    ? generatedHeader(sourceRelative, 'ts', buildCommand, versions, inputs) +
      variantDecl
    : null;
  ...
  const cssContent =
    generatedHeader(sourceRelative, 'css', buildCommand, versions, inputs) + css;
  const jsContent =
    generatedHeader(sourceRelative, 'js', buildCommand, versions, inputs) +
    generateBuiltModule(
      resolvedTheme || themeDef,
      iconInfo,
