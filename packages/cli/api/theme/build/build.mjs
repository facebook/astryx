 ...
 * - A JS module that re-exports the built theme (+ icon registry, and the
 *   theme's custom variants as a runtime `variants` field so `astryx
 *   component` can annotate them)
 ...
 *
 * SYNC: packages/cli/api/theme/build/build.mjs (collectCustomVariants,
...
/**
 * Collect enrolled custom component values across root, onDark, and onLight
 * using the same classification as adaptation validation. Resolve each entry
 * to the public interface that its generated declaration augments.
 *
 * This is the single source of truth for "what custom variants does this
 * theme add": `generateVariantDeclarations` renders the entries as the
 * `.variants.d.ts` module augmentations, and `collectRuntimeVariants` folds
 * them into the built module's runtime `variants` field — so the type surface
 * and what the CLI reports can never disagree.
 *
 * A value only qualifies when `@astryxdesign/core/<Component>` actually
 * exports a matching augmentable interface. Props backed by closed
 * literal-union types (e.g. Button `size`, Heading `level`) have no
 * augmentation point — a custom value there never type-checks, so it is
 * neither declared nor advertised.
 *
 * @param {{components?: Record<string, Record<string, Record<string, unknown>>>}} themeDef - Theme definition (resolved by defineTheme)
 * @returns {Promise<Array<{component: string, prop: string, values: string[], modulePath: string, interfaceName: string}>>}
 */
async function collectCustomVariants(themeDef) {
  ...
  /** @type {Array<{component: string, prop: string, values: string[], modulePath: string, interfaceName: string}>} */
  const entries = [];
  ...
  for (const [component, props] of Object.entries(customValues)) {
    for (const [prop, values] of Object.entries(props)) {
      ...
      entries.push({
        ...
        prop,
        values: [...values],
        modulePath: `@astryxdesign/core/${target.moduleName}`,
...
/**
 * Generate TypeScript declaration content with module augmentation for the
 * custom prop values collected by `collectCustomVariants`.
 *
 ...
 *
 * @param {Awaited<ReturnType<typeof collectCustomVariants>>} entries
 * @returns {string|null} TypeScript declaration content, or null if no augmentations needed
 */
function generateVariantDeclarations(entries) {
  // If every custom value targeted a non-augmentable prop (or there were
  // none), there's nothing to declare — return null so no `.variants.d.ts`
  // is written.
  ...
  for (const entry of entries) {
    sections.push(`declare module '${entry.modulePath}' {`);
    sections.push(`  interface ${entry.interfaceName} {`);
...
/**
 * Fold collected custom variants into the runtime shape the CLI's theme
 * resolver reads: `{ [componentKey]: value[] }`, values deduped across props
 * in declaration order. This is the `variants` contract declared by
 * `resolveTheme()` (clients/cli/lib/resolve-theme.mjs) and consumed by
 * component-format.mjs, which suffixes each value with `*` in the theming
 * targets table. Returns null when the theme adds no custom variants, so the
 * built module only carries the field when it has something to say.
 *
 * @param {Awaited<ReturnType<typeof collectCustomVariants>>} entries
 * @returns {Record<string, string[]>|null}
 */
function collectRuntimeVariants(entries) {
  if (entries.length === 0) return null;

  /** @type {Record<string, string[]>} */
  const variants = {};
  for (const entry of entries) {
    const values = variants[entry.component] || (variants[entry.component] = []);
    for (const v of entry.values) {
 ...
 * override it had.
 *
 * It also carries the theme's custom variants as a runtime `variants` field
 * (`{ [componentKey]: value[] }`, same data the `.variants.d.ts` augments the
 * types with). The CSS doesn't need it either — it exists for the CLI:
 * `resolveTheme()` hands it to `astryx component`, which annotates each
 * theme-added variant with `*` in the theming targets table. Without it a
 * variant the compiler accepts and the CSS renders is invisible to the one
 * tool agents are told to consult.
 *
 ...
 * @param {Record<string, string[]> | null} [variants] - Custom variants from
 *   `collectRuntimeVariants`; emitted as the module's `variants` field.
 * @returns {string}
 */
function generateBuiltModule(themeDef, iconInfo, iconsSpecifier, variants = null) {
  ...
  // Runtime custom variants for the CLI (see the function doc above). Themes
  // without custom variants keep the historical output byte-for-byte.
  const variantsField = serializeField('variants', variants);
...
export const ${toIdentifier(themeDef.name)}Theme = {
  ...
  __built: true,
  tokens: ${tokensStr},
${inheritableFields}${variantsField}${iconsField}
};
${iconReExport}`;
}
...
export async function themeBuild(
  ...
  // before the main .d.ts so the latter can reference it (see below). The
  // same collected entries feed the built module's runtime `variants` field,
  // so the type surface and the CLI's report share one source of truth.
  const augmentationSource = resolvedTheme || themeDef;
  const customVariants = await collectCustomVariants(augmentationSource);
  const variantDecl = generateVariantDeclarations(customVariants);
  ...
  const jsContent =
    ...
    generateBuiltModule(
      ...
      collectRuntimeVariants(customVariants),
    );
  const dtsContent =
    generatedHeader(sourceRelative, 'ts', buildCommand, versions) +
