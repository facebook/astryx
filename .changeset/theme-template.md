...
[feat] `astryx init --features theme` writes an annotated theme template into your project.

`--features theme` printed a one-line hint and wrote nothing, which is the weakest form of the help a theme author needs: the first problem is not knowing the command, it is not knowing what the theme surface contains. It now writes `theme.template.ts` — every `defineTheme` field with a note on when to reach for it, the token families, the component override syntax, and the consumption steps (providing the theme, loading the fonts you name, building for SSR) — each section naming the CLI command that prints its authoritative reference. An existing file is never clobbered.
