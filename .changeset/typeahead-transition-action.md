...
Typeahead could show a search in flight but had no way to say its value was busy. `isLoading` marks the field value as resolving or being saved: the end-lane Spinner shows and the combobox carries `aria-busy`, while the search source and its results stay untouched (FR5). `changeAction` runs after `onChange` with the same proposed item, in a React transition, on both value paths — selection and the clear button — with the proposed item shown as the token until `value` accepts or replaces it; a controlled value that arrives while the Action is still pending wins at once (FR6). A search in flight and a busy value share one Spinner and one `aria-busy`, never two (FR7). Callers that pass neither prop see no change beyond the focus and disabled-reason fixes below.

When the selected token and the input swap places, focus now moves to the one that stays. A focused token withdrawn by an Action that is not accepted, or by the parent clearing the value, hands focus to the input instead of dropping it to the page; a token restored over the focused input, when a clear Action is not accepted, takes focus instead of leaving it on the hidden input.
