...
A signal-terminated run reports too, and reports honestly. Ctrl-C out of a command that does not handle it lands as `error` with `ERR_SIGNAL_TERMINATED` and exit 130. Ctrl-C out of one that does — `theme build --watch` printing "Stopped watching." and returning 0 — keeps the outcome the command actually produced, with `signal` set so those runs are still findable. Watch mode is the ordinary place people press Ctrl-C, and filing every one of those as a failure would bury the real ones.

Hardened against two adversarial chaos runs, each finding mutation-tested before its fix landed: a `__proto__` key silently reparenting the record that carried it, one oversized value discarding the whole event, an exit that bypassed `cliError` being indistinguishable from a classified failure, a signal-terminated run leaving no record at all, a sensitive `--flag=value` scrubbed in `argv` but written back out in full through the error message and captured stderr that quote it, absolute paths surviving inside stack frames (where nothing puts whitespace in front of them) and taking the machine's username with them, and a graceful Ctrl-C being recorded as a failure with an exit code the process never returned.
