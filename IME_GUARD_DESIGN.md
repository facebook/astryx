...
- **Ref tracking** (CodeEditor): track `compositionstart`/`compositionend` to
  gate the _input/change_ path, not a keydown command.
...
- **Wrapping is wrong at 2 sites.** `useFocusTrap` and `Dialog` combine the IME
  check with _other_ conditions (`isTopEscapeHandler`, `defaultPrevented`,
...
tab-moves-focus) and Tab _without_ an IME guard, so a composing Escape/Tab could
