  ...
  render: () => <div {...stylex.props(styles.storyWrapper)}>
      <div>
        ...
        <div {...stylex.props(styles.row)}>
          ...
          <Avatar name="Grace Hopper" src="https://i.pravatar.cc/150?img=5" href="https://example.com/grace" />
        </div>
      </div>
      <div>
        <h4 {...stylex.props(styles.heading)}>As a button (onClick)</h4>
        <div {...stylex.props(styles.row)}>
          <Avatar name="Alan Turing" onClick={() => alert('Open Alan Turing')} />
          <Avatar name="Katherine Johnson" src="https://i.pravatar.cc/150?img=9" onClick={() => alert('Open Katherine Johnson')} />
        </div>
      </div>
    </div>
}`,...x.parameters?.docs?.source},description:{story:"Interactive avatars. `href` renders an `<a>` (native right-click / Cmd+click);\n`onClick` renders a `<button>`. Both get a focus-visible accent ring. A\nmeaningful `name`/`alt` is required so the control has an accessible name.",...x.parameters?.docs?.description}}},S=[`Default`,`WithImage`,`AllSizes`,`WithImages`,`InitialsFallback`,`NoImageNoName`,`FallbackChain`,`WithStatus`,`StatusAcrossAllSizes`,`StatusWithSizes`,`StatusWithIcon`,`NumericSizes`,`Interactive`]}))();export{d as AllSizes,l as Default,h as FallbackChain,p as InitialsFallback,x as Interactive,m as NoImageNoName,b as NumericSizes,_ as StatusAcrossAllSizes,y as StatusWithIcon,v as StatusWithSizes,u as WithImage,f as WithImages,g as WithStatus,S as __namedExportsOrder,c as default};