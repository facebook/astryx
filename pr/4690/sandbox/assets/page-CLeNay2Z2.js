import{q as e}from"./padding.stylex-C-GcuG1E.js";import{t}from"./Text-CLtYjMHW.js";import{t as n}from"./Heading-4uUscgzo.js";import{t as r}from"./Card-BT-dRL-n.js";import{Bt as i,N as a,a as o,c as s,d as c,f as l,i as u,k as d,l as f,o as p,p as m,s as h,u as g}from"./index-CJumU6h3.js";import{t as _}from"./Banner-DVl-rOPZ.js";import{t as v}from"./Badge-C0AePQeD.js";import{t as y}from"./EmptyState-Bq1sLOUo.js";import{a as b,i as x,n as S,r as C,s as w,t as T}from"./Table-DtT3J2-5.js";import{t as E}from"./PageFrame-Dc_Xs3Dm.js";import{a as D,i as O}from"./spring-DdZwaVal.js";import{n as k,s as A}from"./motionAudit-Dzzc8uoB.js";var j=e(),M=new Map(f),N=28;function P(e){return{code:e.map(e=>e.text).join(`
`),highlightLines:e.map((e,t)=>e.added===!0?t+1:0).filter(e=>e>0)}}function F(e){let t=g.map(t=>{let n=u(e(t.name)),r=t.primitive==null?void 0:M.get(t.primitive);if(r!=null&&r===n)return{text:`  '${t.name}': 'var(${t.primitive})',`,added:!0};let i=r==null?` // no primitive: the reduced-motion escape`:` // tuned off ${t.primitive} (${r}ms) — move the primitive or accept the fork`;return{text:`  '${t.name}': '${n}ms',${i}`,added:!0}});return[{text:`// @astryxdesign/core/src/theme/tokens.stylex.ts`},{text:`// Additions only. The nine primitives and --ease-standard do not move.`},{text:``},{text:`export const easeDefaults = {`},{text:`  '${h.name}': '${h.value}', // kept; deprecated in docs, not in code`},...c.map(t=>({text:`  '${t.name}': '${e(t.name)}',`,added:!0})),{text:`} as const;`},{text:``},{text:`export const easeVars = stylex.defineVars(easeDefaults);`},{text:``},{text:`export const durationDefaults = {`},...f.map(([e,t])=>({text:`  '${e}': '${t}ms',`})),{text:``},{text:`  // Semantic layer. Aliased, so the motion scale a theme sets still owns`},{text:`  // these — the same shape typeScaleDefaults uses for type.`},...t,{text:`} as const;`},{text:``},{text:`export const durationVars = stylex.defineVars(durationDefaults);`},{text:``},{text:`// Stagger is a delay between siblings, not a duration, so it gets its own`},{text:`// group rather than widening DurationVarName.`},{text:`export const staggerDefaults = {`,added:!0},...m.map(([t])=>({text:`  '${t}': '${u(e(t))}ms',`,added:!0})),{text:`} as const;`,added:!0},{text:``,added:!0},{text:`export const staggerVars = stylex.defineVars(staggerDefaults);`,added:!0}]}function I(e,t){let n=e=>e.replace(/^--(duration|ease|stagger)-/,``),r=t=>Number((u(e(t))/1e3).toFixed(3)),i=t=>{let n=e(t);return O(n)?JSON.stringify(n):`[${D(n).join(`, `)}]`};return`// @generated from the Astryx theme by scripts/generate-motion-mirror.mjs.
// Values are resolved through resolveThemeTokens, so a theme that retunes the
// motion scale retunes this file too.

export const duration = {
${g.map(e=>`  ${n(e.name)}: ${r(e.name)},`).join(`
`)}
} as const;
