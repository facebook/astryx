import{n as e}from"./rolldown-runtime-DaJ6WEGw.js";import{t}from"./react-DvlgmmzG.js";import{o as n}from"./useTheme-CQdBTfQJ.js";import{t as r}from"./jsx-runtime-cM__dR4X.js";import{_ as i,a,c as o,d as s,f as c,g as l,h as u,i as d,m as f,n as p,o as m,p as h,r as g,s as _,t as v,u as y,x as b,y as x}from"./theme-B8zE3zLJ.js";import{i as S,t as C}from"./CodeBlock-KmyHdAfT.js";var w,T,E,D,O,k,A,j,M,N,P,F,I,L,R,z,B,V,H,U,W,G,K;e((()=>{t(),C(),p(),v(),w=r(),T=[`import {useState, useEffect} from 'react';`,``,`interface User {`,`  id: string;`,`  name: string;`,`  roles: string[];`,`}`,``,`const API_URL = "https://api.example.com";`,`const MAX_RETRIES = 3;`,``,`// Fetch user data with retry logic`,`async function fetchUser(id: string): Promise<User> {`,"  const response = await fetch(`${API_URL}/users/${id}`);",`  if (!response.ok) {`,"    throw new Error(`HTTP ${response.status}`);",`  }`,`  return response.json();`,`}`,``,`export function UserCard({id}: {id: string}) {`,`  const [user, setUser] = useState<User | null>(null);`,``,`  useEffect(() => {`,`    fetchUser(id).then(setUser);`,`  }, [id]);`,``,`  if (!user) return <div>Loading...</div>;`,``,`  return (`,`    <div className="card">`,`      <h2>{user.name}</h2>`,`      <span>{user.roles.length} roles</span>`,`    </div>`,`  );`,`}`].join(`
