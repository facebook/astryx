 ...
 * Generated cover for release posts: the version set in the Astryx brand blue
 * on a soft pastel-blue field (the same token as the landing feature cards),
 * with the Astryx mark as the separator between version segments and the
 * package name beneath.
 *
 * Text is rendered with the XDS `Text` component using the defined type-scale
 * `code` type + `size` tokens (theme code font), so the cover stays on the
 * design system's typography rather than ad-hoc CSS. Like BlogCoverArt it
 * renders inline (not via <img>).
 *
...
// Astryx icon mark geometry (40x40 viewBox), mirrored from BlogCoverArt.
const MARK_PATH =
...
// The field reuses the same soft pastel blue as the landing-page feature cards
// (--astryx-marketing-feature-card-bg, globals.css). Version + mark icons use
// the Astryx brand blue; the package label uses the secondary text color.
...
const styles = stylex.create({
  root: {
    width: '100%',
    height: '100%',
    // Establish a query container so the version + marks scale to fill the
    // cover at any size (small card, feature card, article hero) — the type
    // scale tops out too small to fill the hero, so size fluidly instead.
    containerType: 'size',
    backgroundColor: FIELD_BLUE,
    // The field is a fixed light blue, so pin light-dark() tokens to their
    // light values — otherwise text/brand tokens flip in dark mode and lose
    // contrast against the light field.
    colorScheme: 'light',
    display: 'flex',
  ...
  version: {
    ...
    // ~2/3 of the original 22cqw so the version reads smaller while still
    // scaling with the cover.
    fontSize: '14.7cqw',
    lineHeight: 1,
  },
  handle: {
  ...
  mark: {
    ...
    // Drop the mark toward the baseline so it reads as the "dot" between
    // version segments rather than a centered glyph.
