...
description: 'We gave four models running on a laptop the same page to build with Astryx. Three shipped it on the first try; one could not load at all.'
date: '2026-08-11'
type: 'engineering'
authors:
  - 'humbertovirtudes'
...
Three of the four models built the page on the **first** attempt. No fix-up rounds needed.
...
| Muse Glimmer         | 28B                  | —           | —          | did not load       |

The headline is that a 7.5B model small enough to run on a laptop with room to spare got a working Astryx page on the first try. The CLI-derived prompt did its job: none of the three that ran hallucinated a component or invented a prop badly enough to break the build.

Speed and quality did not move together. Nemotron is a mixture-of-experts model — 30B of weights but only about 3B active per token — so it ran fastest at 69.4 tokens/second while producing the most output (7,691 tokens; it reasons out loud before writing code). Qwen was the slowest at 11.3 tokens/second, roughly six times slower than Nemotron for a similar-length page. If you are choosing a local model for this kind of work, an MoE model is worth a look: you get the judgment of a large model closer to the speed of a small one.
...
Qwen and Nemotron produced clean Astryx: layout through `VStack`, `HStack`, and `Grid`; spacing through the numeric scale; color through `Card` variants. No raw HTML layout, no inline styles.
...
**A model can be too new to run.** The fourth model, Muse Glimmer, never produced a single line. LM Studio's current llama.cpp runtime (2.13.0, the latest available to me) does not recognize its `muse-glimmer` architecture, so the model fails to load outright:

```
error loading model architecture: unknown model architecture: 'muse-glimmer'
```

Nothing to do with Astryx, and nothing I could work around — the model's format is simply ahead of the runtime that has to read it. Worth remembering when a shiny new local model refuses to start: the bottleneck may be the runtime, not the model.
...
The honest caveats hold too: a clean build is not clean code, small models still reach for wrapper divs when unsure, and your local runtime can quietly cap your context or refuse a model entirely. But the core answer is yes — the same CLI that helps a frontier agent build with Astryx helps the model on your laptop do it too.
