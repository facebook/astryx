...
**Layout under long strings.** Our pseudo-locale today swaps letters for accented forms and wraps each string in visible markers, which is great for spotting untranslated text and narrow-column breakage. What it doesn't yet do is _inflate length_ – the classic pseudo-translation trick of multiplying string length to stress-test layouts before real translations exist. Adding that would catch a class of layout bug we currently only find once a long-language translation lands.
