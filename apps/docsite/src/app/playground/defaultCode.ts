 ...
 *
 * A self-contained, no-prop adaptation of ThemeCardShowcase so it runs in the
 * preview sandbox (see (preview)/playground-preview/scope.ts). Kept as a string
 * because the playground edits/compiles it as user code rather than importing it.
 *
 * The sandbox mocks @stylexjs/stylex: `stylex.props()` returns no styles, so raw
 * elements (div/img/span) must apply `styles.X` via the `style` prop. `xstyle`
 * on XDS components still works because the real stylex runtime inside @xds/core
 * treats the plain style objects as inline styles.
 */

export const DEFAULT_CODE = `import {type ReactNode} from 'react';
  ...
  return (
    <XDSCard padding={5} xstyle={styles.activityCard}>
      <XDSVStack gap={4} xstyle={styles.activityCardStack}>
        ...
        <XDSVStack gap={2}>
          <div style={styles.sparkline} aria-hidden="true">
            {SPARKLINE_DATA.map((value, i) => (
              <div
                key={i}
                style={{...styles.sparkBar, height: value + '%'}}
              />
            ))}
          </div>
          <div style={styles.sparkLabels} aria-hidden="true">
        ...
        <XDSGrid columns={2} gap={3}>
          <XDSVStack gap={0}>
            <span style={styles.kpiValue}>18K</span>
            <XDSText type="supporting" color="secondary">
              Monthly revenue
            </XDSText>
          </XDSVStack>
          <XDSVStack gap={0}>
            <span style={styles.kpiValue}>+12%</span>
            <XDSText type="supporting" color="secondary">
              Order growth
            </XDSText>
          </XDSVStack>
        ...
        <XDSVStack gap={1} xstyle={styles.activityListFade}>
          {ACTIVITY.map(item => (
            <XDSItem
              key={item.id}
              media={
                <div style={styles.activityIcon} aria-hidden="true">
                  {item.icon}
                </div>
              }
              label={item.label}
  ...
  return (
    <XDSHStack gap={3} vAlign="center">
      {thumbnailSrc ? (
        ...
        <div style={styles.thumbnailFallback} aria-hidden="true">
...
function InventoryCard({images}: {images: Record<string, string>}) {
  return (
    <XDSCard padding={0} xstyle={styles.inventoryCard}>
      ...
      {LOW_STOCK_COUNT > 0 && (
        <div style={styles.inventoryBannerWrap}>
          <XDSBanner
            status="warning"
