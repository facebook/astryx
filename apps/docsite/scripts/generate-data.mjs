...
function generateGalleryShowcaseRegistry(showcaseEntries) {
  ...
  const importLines = eager
    .map(e => `import ${e.basename} from './showcases/${e.basename}';`)
    .join('\n');
  const mapLines = eager
    .map(e => `  '${e.name}': ${e.basename},`)
    .join('\n');
...
`;
  ...
  writeRegistry('galleryShowcaseRegistry.ts', content);
  console.log(`  ${eager.length} eager showcases: ${eager.map(e => e.name).join(', ')}`);
  return eager.length;
}
...
async function main() {
  ...
  console.log(`  ${blockCount} blocks (${showcaseCopied.length} showcases, ${examplesCopied} examples)`);
  console.log(`  ${eagerCount} eager gallery showcases`);
