 ...
 *   - docTopicSlugs.ts     — docs topic slugs only, for client bundles
 */
...
async function generateDocsRegistry() {
  ...
  if (!fs.existsSync(DOCS_DIR)) {
    ...
    writeRegistry('docTopicSlugs.ts', `// Auto-generated — no docs found\nexport const docTopicSlugs: string[] = [];\n`);
...
`;
  ...
  // Slugs-only sibling of docsRegistry.ts. Client modules (e.g. the inline
  // markdown renderer, which links `astryx docs <topic>` code spans to their
  // pages) gate on known topics via this file so they don't pull the full
  // registry — every section's content — into the browser bundle.
  const slugsContent = `// Auto-generated by scripts/generate-data.mjs — do not edit

export const docTopicSlugs: string[] = ${JSON.stringify(
    docTopics.map(d => d.topic),
    null,
    2,
  )};
`;
  writeRegistry('docTopicSlugs.ts', slugsContent);
