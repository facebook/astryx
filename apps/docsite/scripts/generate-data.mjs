...
// CONTENT ROOT is target-dependent (canary → workspace/main, latest → pinned
// published version). The docsite CODE is always this checkout; only the
// package sources we read documentation from vary. See resolve-content-root.mjs.
const BUILD = resolveContentRoot();
const REPO_ROOT = BUILD.contentRoot;
const CLI_ROOT = BUILD.cliRoot;
console.log(
  `Content target: ${BUILD.target} (${BUILD.label}) — reading from ${path.relative(DOCSITE_ROOT, REPO_ROOT) || REPO_ROOT}`,
);
...
// Emits the current content target so the UI can label which version the
// docs describe and offer a switcher between deployed builds.
function generateBuildTargetRegistry() {
  console.log('Generating build-target registry...');
  const coreVersion =
    BUILD.versions?.['@astryxdesign/core'] ??
    (() => {
      try {
        return JSON.parse(
          fs.readFileSync(
...
export type DocsTarget = 'canary' | 'latest';

export interface BuildTarget {
  /** Which content line this build documents. */
  target: DocsTarget;
  /** Human-readable label for the version switcher. */
  label: string;
  /** Resolved @astryxdesign/core version documented by this build, if known. */
  coreVersion: string | null;
}

export const buildTarget: BuildTarget = ${JSON.stringify(
    {target: BUILD.target, label: BUILD.label, coreVersion: coreVersion ?? null},
    null,
    2,
  )};
`;
  writeRegistry('buildTarget.ts', content);
}

async function main() {
  console.log('Generating docsite data...\n');

  generateBuildTargetRegistry();
