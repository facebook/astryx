...
async function generateTemplateRegistry() {
  ...
  const templates = [];
  // Slugs of templates that have a live page.tsx, used to generate the
  // lazy component loader map below. Collected from the same directory
  // scan so the registry and the component map never drift apart.
  ...
  for (const dir of dirs) {
    ...
    componentSlugs.push(dir.name);
  }

  templates.sort((a, b) => a.name.localeCompare(b.name));
  componentSlugs.sort((a, b) => a.localeCompare(b));

  writeTemplateComponentRegistry(componentSlugs, PAGES_DIR);
...
/**
 * Emit a lazy-loaded component map for every page template, keyed by slug.
 *
 * Both the gallery thumbnails (TemplateThumbnail) and the preview dialog
 * (TemplatePreviewSurface) render a template's real `page.tsx`. Rather
 * than hand-maintaining a `lazy(() => import(...))` map — which silently
 * goes stale whenever a template is added or removed — we generate it from
 * the same directory scan that produces templateRegistry.ts, so the two
 * always stay in sync. Mirrors the showcase/example loader registries.
 */
function writeTemplateComponentRegistry(slugs, pagesDir) {
  ...
  const importLines = slugs
    .map(slug => {
      ...
      return `  ${JSON.stringify(slug)}: lazy(() => import(${JSON.stringify(rel)})),`;
    })
    .join('\n');
...
/**
 * Lazy-loaded registry of template page components, keyed by slug.
 * Generated from packages/cli/templates/pages/ — add a template there
 * and it appears here automatically on the next \`yarn generate\`.
 */
...
`;
  writeRegistry('templateComponentRegistry.ts', content);
}
