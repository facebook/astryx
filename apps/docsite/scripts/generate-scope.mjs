...
lines.push("  props: (...styles: unknown[]) => {");
lines.push("    const merged: Record<string, unknown> = {};");
lines.push("    for (const s of styles) {");
lines.push("      if (s && typeof s === 'object') {");
lines.push("        for (const [key, value] of Object.entries(s as Record<string, unknown>)) {");
lines.push("          if (typeof value === 'object' && value !== null) {");
lines.push("            const inner = value as Record<string, unknown>;");
lines.push("            if ('default' in inner) { merged[key] = inner.default; }");
lines.push("          } else if (value != null) { merged[key] = value; }");
lines.push("        }");
lines.push("      }");
lines.push("    }");
lines.push("    return {style: merged};");
lines.push("  },");
