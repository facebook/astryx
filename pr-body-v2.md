...
### `useXDSStreamingText` — new core hook

Smooths bursty LLM token delivery into a steady character-by-character reveal. Lives in `@xds/core/hooks`.

- Derives animation timing from XDS motion tokens (`--duration-fast-min`) via `useXDSTheme`
- Falls back to sensible defaults when no `XDSTheme` provider is present
...
import { useXDSStreamingText } from '@xds/core';
...
function AIResponse({ text, isStreaming }) {
  const displayed = useXDSStreamingText(text, isStreaming);
...
- **useXDSStreamingText**: 6 tests — non-streaming passthrough, instant speed, streaming start/stop, reset, progressive reveal
- **XDSMarkdown**: 15 tests — every element type, heading clamping, streaming cursor, component overrides, link behavior

### What's next

- `useXDSLinkify` hook (separate PR) — auto-detect URLs + custom patterns (T1234, D5678) in plain text, render via XDSLink
- Storybook stories, XDSLinkProvider integration docs
